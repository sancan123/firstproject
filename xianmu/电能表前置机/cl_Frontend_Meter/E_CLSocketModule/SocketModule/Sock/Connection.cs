...
namespace E_CLSocketModule.SocketModule.Sock
{
    ...
    internal class Connection
    {
        ...
        public Connection(IPAddress remoteIp, int remotePort, int localPort, int intBasePort,string HaveProtocol, int MaxWaitMSecond, int WaitSecondsPerByte)
        {
            connection = new UDPClient(remoteIp.ToString().Split(':')[0], localPort, remotePort, intBasePort,HaveProtocol) as IConnection;
        ...
        public bool Send(Packet.SendPacket sendPack, Packet.RecvPacket recvPack, string PortName)
        {
            ...
            if (!Send(ref vData, sendPack.IsNeedReturn,sendPack.WaiteTime(),sendPack.MaxByte()))
            {
        ...
        /// <param name="PortName">仅用于报文日志，无实际端口作用</param>
        /// <returns></returns>
        public bool Send(Packet.SendPacket sendPack, Packet.RecvPacket recvPack, string PortName, int MaxWaitSeconds)
        {
            ...
            Console.WriteLine("SendData:({0}) {1}", sendPack.GetPacketName(), BitConverter.ToString(vData, 0));
            if (!Send(ref vData, sendPack.IsNeedReturn, sendPack.WaiteTime(), sendPack.MaxByte(), MaxWaitSeconds))
            {
        ...
        /// <returns></returns>
        private bool Send(ref byte[] vData, bool isNeedReturn, int WaiteTime, int MaxByte)
        {
            ...
            lock (objSendLock)
            {
                if (connection != null)
                {
                    if (!connection.SendData(ref vData, isNeedReturn, WaiteTime,MaxByte))
                        return false;
                }
        ...
        /// <param name="WaiteTime"></param>
        /// <returns></returns>
        private bool Send(ref byte[] vData, bool isNeedReturn, int WaiteTime, int MaxByte, int MaxWaitSeconds)
        {
            if (connection == null) return false;
            lock (objSendLock)
            {
                if (connection != null)
                {
                    if (!connection.SendData(ref vData, isNeedReturn, WaiteTime, MaxByte,MaxWaitSeconds))
                        return false;
                }
                if (isNeedReturn && vData.Length < 1)
                {
