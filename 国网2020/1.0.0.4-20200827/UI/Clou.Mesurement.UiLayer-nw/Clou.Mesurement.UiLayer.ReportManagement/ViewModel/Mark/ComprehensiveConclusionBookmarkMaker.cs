...
namespace Mesurement.UiLayer.DataManager.Mark.ViewModel
{
    ...
    /// 检定结论书签制作器
    /// </summary>
    class ComprehensiveConclusionBookmarkMaker : ViewModelBase
    {
        #region 检定点相关
        ...
        public string BookmarkName
        {
            get
            {
                ...
                {
                    bookmarkName = string.Format("表{0}VX{1}VX{2}VX{3}", 0, ItemKey, ResultName, Format);
                }
                return bookmarkName;
            }
        }
        ...
        public string SummaryMarkName
        {
            get
            {
                ...
                {
                    //summaryMarkName = string.Format("表{0}VX{1}", 0, CategoryNo);
                    summaryMarkName = string.Format("已选表VX{0}VX{1}", CategoryNo, Format);
                }
