...
{
    public class UserCardFileParam
    {
        ...
        private byte[] _meterNo = new byte[6];
        public string meterNo
        {
            get
            {
                return BitConverter.ToString(_meterNo).Replace("-", "");
            }
            set
            {
                ...
                {
                    Tools.DataConvert tdc = new Tools.DataConvert();
                    _meterNo = tdc.stringToByte(value);
                }
                else
                {
                    throw new Exception("数据格式错误!");
        ...
        private byte[] _userNo = new byte[6];
        public string userNo
        {
            get
            {
                return BitConverter.ToString(_userNo).Replace("-", "");
            }
            set
            {
                if (!string.IsNullOrEmpty(value) && value.Length == 12)
                {
                    Tools.DataConvert tdc = new Tools.DataConvert();
                    _userNo = tdc.stringToByte(value);
                }
                else
                {
                    throw new Exception("数据格式错误!");
        ...
        private byte _userCardType = 0x01;
        public string userCardType
        {
            get
            {
                return Convert.ToString(_userCardType, 16).PadLeft(2, '0');
            }
            set
            {
                if (!string.IsNullOrEmpty(value) && value.Length <= 2)
                {
                    _userCardType = byte.Parse(value, System.Globalization.NumberStyles.HexNumber);
                }
                else
                {
                    throw new Exception("数据格式错误!");
        ...
        public byte[] GetData()
        {
            ...
            Array.Copy(_meterNo, 0, tmp, 26, 6);
            Array.Copy(_userNo, 0, tmp, 32, 6);
            tmp[38] = _userCardType;
            return tmp;
        }
    }
}
