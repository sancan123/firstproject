...
namespace CLDC_VerifyAdapter.MeterClearZero
{
    ...
    /// 计量清零操作
    /// </summary>
    class MeterClearEnergyJ : VerifyBase
    {
        public MeterClearEnergyJ(object plan)
            : base(plan)
        {
        ...
        public override void Verify()
        {
            ...
            //读事件记录
            if (Stop) return;
            MessageController.Instance.AddMessage("正在读取电表校时记录总次数");
            string[] strLoseTimeQ1 = MeterProtocolAdapter.Instance.ReadData("03300400", 3);
            ...
            for (int i = 0; i < BwCount; i++)
            {
                ...
                if (strLoseTimeQ1[i] == "000000")
                {
                    ResultDictionary["结论"][i] = Variable.CTG_BuHeGe;
                    reasonS[i] = "清零后，读取事件记录数据不对";
                    continue;
                }
                 if ((!string.IsNullOrEmpty(strLoseCountQ[i]) && !string.IsNullOrEmpty(strLoseCountH[i]) && Convert.ToInt32(strLoseCountQ[i]) + 1 == Convert.ToInt32(strLoseCountH[i])) || fClear[i])
                {
                    ResultDictionary["结论"][i] = Variable.CTG_BuHeGe ;
                    reasonS[i] = "只有工装模块下电表不应该清零！";
                }
                else
                {
