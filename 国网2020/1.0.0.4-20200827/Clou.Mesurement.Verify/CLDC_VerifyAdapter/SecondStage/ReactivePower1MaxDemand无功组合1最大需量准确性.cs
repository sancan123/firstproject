...
{
    class ReactivePower1MaxDemand:VerifyBase
    {


           #region ----------构造函数----------

        public ReactivePower1MaxDemand(object plan)
            : base(plan)
        {
        }
        ...
        public override void Verify()
        {
            ...
            Cus_PowerFangXiang Wglfx1 = Cus_PowerFangXiang.第一象限无功;
            Cus_PowerFangXiang Wglfx2 = Cus_PowerFangXiang.第四象限无功;
             for (int i = 0; i < BwCount; i++)
            {
                if (Helper.MeterDataHelper.Instance.Meter(i).YaoJianYn)
                {
                    if (flt_TzzW1[i] == "41")
                    {
                        Wglfx1 = Cus_PowerFangXiang.第一象限无功;
                        Wglfx2 = Cus_PowerFangXiang.第四象限无功;
                    }
                    else
                    {
                        Wglfx1 = Cus_PowerFangXiang.第一象限无功;
                        Wglfx2 = Cus_PowerFangXiang.第二象限无功;
                    }
                }
             }
           ...
           MessageController.Instance.AddMessage("开始做第一象限无功最大需量");
           ...
           string[] StrRemand = new string[BwCount];
           while (true)
           {
               ...
               float curStandMeterP = GlobalUnit.g_StrandMeterP[0];
                 ...
                 if (tempMinute > PastMinute)
                 {
                     ...
                     StrRemand = MeterProtocolAdapter.Instance.ReadData("01050000", 8);
                     for (int i = 0; i < BwCount; i++)
                     {
                         if (Helper.MeterDataHelper.Instance.Meter(i).YaoJianYn)
                         {
           ...
           float[] DemandValueW1 = MeterProtocolAdapter.Instance.ReadDemand((byte)2, 0x00);
           StrRemand = MeterProtocolAdapter.Instance.ReadData("01080000", 8);
           float XLwc = 0;
