...
{
    class MaxDemandByError:VerifyBase
    {


           #region ----------构造函数----------

        public MaxDemandByError(object plan)
            : base(plan)
        {
        }
        ...
        public float GetErrorLevel1(float meterLevel)
        {
      ...
      Cus_PowerFangXiang      glfx = Cus_PowerFangXiang.正向有功;
            string fx = "";
           if (!string.IsNullOrEmpty(arrayErrorPara[0]))
            {
                switch (arrayErrorPara[0])
                {
                    case "正向有功":
           ...
           float _xIb = Number.GetCurrentByIb(arrayErrorPara[1], Helper.MeterDataHelper.Instance.Meter(GlobalUnit.FirstYaoJianMeter).Mb_chrIb, Helper.MeterDataHelper.Instance.Meter(GlobalUnit.FirstYaoJianMeter).Mb_BlnHgq);
           if (Helper.EquipHelper.Instance.PowerOn(GlobalUnit.U, _xIb, 1, (int)glfx,fx+ "1.0", true, false) == false)
           {
               Stop = true;
               MessageController.Instance.AddMessage("开始做最大需量");
           ...
           float errorLevel = GetErrorLevel1(curLevel);
           for (int i = 0; i < BwCount; i++)
           {
               if (Helper.MeterDataHelper.Instance.Meter(i).YaoJianYn)
               {
