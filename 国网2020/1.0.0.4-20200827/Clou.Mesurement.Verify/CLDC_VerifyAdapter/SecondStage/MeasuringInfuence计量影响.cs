...
{
    class MeasuringInfuence : VerifyBase
    {
        #region ----------变量声明----------
        ...
        #region ----------构造函数----------
        public MeasuringInfuence(object plan)
            : base(plan)
        {
            //检测检定参数是否正确
            // m_VerifyDemo.BWCount = BwCount;
            ResultNames = new string[] { "功率元件", "功率方向", "电流倍数", "功率因数", "误差下限", "误差上限", "误差圈数", "实际误差", "读取电量",  "结论", "不合格原因" };
        }
        #endregion
      ...
      //      Helper.VerifyDemoHelper.Instance.Reset();
            ...
            while (!m_CheckOver)
            {
                ...
                float[] strErrors = MeterProtocolAdapter.Instance.ReadData("00000000", 4, 2);
                #region
                ...
                //end 读取自监测误差值
                    ...
                    for (int i = 0; i < BwCount; i++)
                    {
                        ...
                        {
                            ResultDictionary["实际误差"][i] = arrayTemp[0];//误差1

                        }
                        ResultDictionary["结论"][i] = tmpErrorConc[i]; 
                        ResultDictionary["读取电量"][i] = strErrors[i].ToString();
                                                                                                                      
                    }
                    MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "实际误差", ResultDictionary["实际误差"]);
                    MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "读取电量", ResultDictionary["读取电量"]);
