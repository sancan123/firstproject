...
namespace CLDC_VerifyAdapter.SecondStage
{
    ...
    /// 组合有功设置
    /// </summary>
    class ActivePowerSettings:VerifyBase
    {


           #region ----------构造函数----------

        public ActivePowerSettings(object plan)
            : base(plan)
        {
        }
        ...
        public override void Verify()
        {
           ...
           string[] Fail = new string[BwCount];
            ...
            MessageController.Instance.AddMessage("正在读取组合有功电量");
            float[] flt_DL = MeterProtocolAdapter.Instance.ReadData("00000000", 4, 2);
            MessageController.Instance.AddMessage("正在读取组合有功特征字");
            string[] flt_Tzz1 = MeterProtocolAdapter.Instance.ReadData("04000601", 1);
            for (int i = 0; i < BwCount; i++)
            {
                if (Helper.MeterDataHelper.Instance.Meter(i).YaoJianYn)
                {
                    //"功率元件","功率方向","电流倍数","功率因数","误差下限","误差上限","误差圈数"
                    ResultDictionary["测试前组合有功电量"][i] = flt_DL[i].ToString();
                    ResultDictionary["测试前组合有功特征字"][i] = flt_Tzz1[i].ToString();    
                   
                }
            }
            MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "测试前组合有功电量", ResultDictionary["测试前组合有功电量"]);
            MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "测试前组合有功特征字", ResultDictionary["测试前组合有功特征字"]);
            ...
            Common.Memset(ref strDataCode, "04000601");
            Common.Memset(ref strData, "04000601" + writedata);
            ...
            MessageController.Instance.AddMessage("正在读取组合有功特征字");
            string[] flt_Tzz2 = MeterProtocolAdapter.Instance.ReadData("04000601", 1);
            ...
            for (int i = 0; i < BwCount; i++)
            {
                ...
                {
                    //"功率元件","功率方向","电流倍数","功率因数","误差下限","误差上限","误差圈数"
                    ResultDictionary["设置后组合有功特征字"][i] = flt_Tzz2[i].ToString();
                   
                }
            }
            MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "设置后组合有功特征字", ResultDictionary["设置后组合有功特征字"]);
            if (Stop) return;
            #region 走字 正向有功
            ...
            #region 走字 反向有功
            MessageController.Instance.AddMessage("最大电流进行反向有功走字20S，请稍候......");
            bool ret1 = Helper.EquipHelper.Instance.PowerOn(GlobalUnit.U, GlobalUnit.Imax, 1, 2, "1.0", true, false);
            if (!ret1)
            {
                throw new Exception("升源失败");
            }
            ...
            MessageController.Instance.AddMessage("正在读取正向有功电量");
            float[] flt_DLY = MeterProtocolAdapter.Instance.ReadData("00010000", 4, 2);
            for (int i = 0; i < BwCount; i++)
            {
                if (Helper.MeterDataHelper.Instance.Meter(i).YaoJianYn)
                {
                    //"功率元件","功率方向","电流倍数","功率因数","误差下限","误差上限","误差圈数"
                    ResultDictionary["正向有功电能"][i] = flt_DLY[i].ToString();

                }
            }
            MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "正向有功电能", ResultDictionary["正向有功电能"]);
            if (Stop) return;
            PowerOn();
            MessageController.Instance.AddMessage("正在读取反向有功电量");
            float[] flt_DLF = MeterProtocolAdapter.Instance.ReadData("00020000", 4, 2);
            ...
            for (int i = 0; i < BwCount; i++)
            {
                ...
                {
                    //"功率元件","功率方向","电流倍数","功率因数","误差下限","误差上限","误差圈数"
                    ResultDictionary["反向有功电能"][i] = flt_DLF[i].ToString();

                }
            }
            MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "反向有功电能", ResultDictionary["反向有功电能"]);
            ...
            for (int i = 0; i < BwCount; i++)
            {
                ...
                {
                    //"功率元件","功率方向","电流倍数","功率因数","误差下限","误差上限","误差圈数"
                    ResultDictionary["测试后组合有功电量"][i] = flt_DLZ[i].ToString();
            ...
            string writedata1 = FormatWriteData("05", "NN", 1, 0);
            Common.Memset(ref strDataCode, "04000601");
            Common.Memset(ref strData, "04000601" + writedata1);
            ...
            MessageController.Instance.AddMessage("正在读取组合有功特征字");
            string[] flt_Tzz3 = MeterProtocolAdapter.Instance.ReadData("04000601",1);
            if (Stop) return;
            MessageController.Instance.AddMessage("正在读取组向有功电量");
            ...
            for (int i = 0; i < BwCount; i++)
            {
                ...
                {
                    //"功率元件","功率方向","电流倍数","功率因数","误差下限","误差上限","误差圈数"
                    ResultDictionary["组合有功特征字"][i] = flt_Tzz3[i].ToString();
                    ResultDictionary["组合有功电量"][i] = flt_DLZ1[i].ToString();

                }
            }
            MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "组合有功特征字", ResultDictionary["组合有功特征字"]);
            MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "组合有功电量", ResultDictionary["组合有功电量"]);
                 ...
                 for (int i = 0; i < BwCount; i++)
            {
                if (Helper.MeterDataHelper.Instance.Meter(i).YaoJianYn)
                {
                    ...
                    else if (flt_DLZ[i] - flt_DLY[i] <=0.1)
                    {
