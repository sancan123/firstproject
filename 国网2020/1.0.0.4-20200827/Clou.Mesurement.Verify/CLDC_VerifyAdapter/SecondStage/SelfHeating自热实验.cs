...
namespace CLDC_VerifyAdapter.SecondStage
{
    class SelfHeating : VerifyBase
    {
        ...
        #region ----------构造函数----------
        public SelfHeating(object plan)
            : base(plan)
        {
            //检测检定参数是否正确
            // m_VerifyDemo.BWCount = BwCount;
            ResultNames = new string[] { "测试时间", "参比误差", "误差值", "最大误差", "最小误差", "误差偏移量(最大误差)", "误差偏移量(最小误差)", "不合格原因", "结论" };
        }
        #endregion
        ...
        public override void Verify()
        {
            ...
            if (verPlan[1] == "0")
            {
                MessageController.Instance.AddMessage("自热时间不能为0");
                Stop = true;
            }
            else if (verPlan[2] == "0")
            {
                WuC = 0;
                MessageController.Instance.AddMessage("误差次数不能为0");
                Stop = true;
            }
            else
            {
                ZRtime = int.Parse(verPlan[1]);
                WuC = int.Parse(verPlan[2]);
                JGtime = ZRtime / WuC;
            }

            if (Stop) return;
            float[] MaxWc = new float[BwCount];
            float[] MinWc = new float[BwCount];
            float[] FirstWc = new float[BwCount];
            ...
            string[,] meterWc = new string[BwCount, WuC+1];
            m_StartTime = DateTime.Now;
            float ENDTIME = 0;
              float pastMinute = 0;
            for (int MeterCount = 0; MeterCount < (WuC+1); MeterCount++)
            {
      ...
      //      Helper.VerifyDemoHelper.Instance.Reset();
         //   m_StartTime = DateTime.Now;                                             //记录下检定开始时间
            ...
            {
                PulseClone[i] = curPoint.LapCount;
            }
            ...
            int maxtime = JGtime * 60*1000;
            int testTime = JGtime * 60 * 1000;
            if (MeterCount != 0)
            {
                if (MeterCount == 1)
                {
            ...
            while (maxtime > 1)
            {
                pastMinute = base.VerifyPassTime / 60F ;
                string strDes = "自热运行等待时间" + (ZRtime).ToString("F2") + "分，已经经过" + pastMinute.ToString("F2") + "分";
                MessageController.Instance.AddMessage(strDes);
                Thread.Sleep(2000);
                maxtime -= 2000;
                if (Stop) break;
                if (pastMinute * 60 * 1000 >= testTime * (MeterCount + 1))
                 {
            ...
            int waitTimeForErr =JGtime* 60;
            m_WCMaxSeconds = m_WCMaxSeconds + waitTimeForErr;
            if (MeterCount == 0)
            {
                MessageController.Instance.AddMessage("开始读参比误差");
            }
            else
            {
                MessageController.Instance.AddMessage("开始读第" + (MeterCount) + "个误差");
            }
            ...
            while (!m_CheckOver)
            {
                ...
                if (base.VerifyPassTime - (pastMinute  *60)> m_WCMaxSeconds &&
            ...
            for (int i = 0; i < BwCount; i++)
            {
                ...
                if (arrayTemp.Length > 2)
                {
                    ...
                    if (MeterCount == 0)
                    {
                        ResultDictionary["参比误差"][i] = meterWc[i, MeterCount];
                       
                        FirstWc[i] = float.Parse(meterWc[i, MeterCount]);
                    }else  if (MeterCount == 1)
                    {
                        ResultDictionary["误差值"][i] = meterWc[i, MeterCount] ;
                        MaxWc[i] = float.Parse( meterWc[i, MeterCount]);
                        MinWc[i] = float.Parse(meterWc[i, MeterCount]);
                     //   FirstWc[i] = float.Parse(meterWc[i, MeterCount]);
                    }
                    else
                    {
                        if (MaxWc[i] < float.Parse(meterWc[i, MeterCount]))
                        {
                            MaxWc[i] = float.Parse(meterWc[i, MeterCount]);
                        }
                        if (MinWc[i] > float.Parse(meterWc[i, MeterCount]))
                        {
                             MinWc[i] = float.Parse(meterWc[i, MeterCount]);
                        }
                        
                        ResultDictionary["误差值"][i] = ResultDictionary["误差值"][i] + "," + meterWc[i, MeterCount];
                    }
                    if (MeterCount == 0)
                    {
                        UploadTestResult("参比误差");
                    }
                    UploadTestResult("误差值");
                }
            }
            errorTable = null;
            ...
            float[] WCPYL = new float[BwCount];
            float[] WCPYL2 = new float[BwCount];
            for (int g = 0; g <WuC; g++)
            {
            ...
            for (int f = 0; f < BwCount; f++)
            {
                ...
                if (result[f])
                {
                    ...
                    WCPYL[f] = MaxWc[f] - FirstWc[f];
                    WCPYL2[f] = MinWc[f] - FirstWc[f];
                    ResultDictionary["最大误差"][f] = MaxWc[f].ToString("0.0000");
                    ResultDictionary["最小误差"][f] = MinWc[f].ToString("0.0000");
                    ResultDictionary["误差偏移量(最大误差)"][f] = WCPYL[f].ToString("0.0000");
                    ResultDictionary["误差偏移量(最小误差)"][f] = WCPYL2[f].ToString("0.0000");
                    if (Math.Abs(WCPYL[f]) > (CurPlan.ErrorShangXian * 0.1) || Math.Abs(WCPYL2[f]) > (CurPlan.ErrorShangXian * 0.1))
                    {
                        ResultDictionary["结论"][f] = "不合格";
                        reasonS[f] = "误差偏移量超差";
                    }
                    else if (Math.Abs(MaxWc[f]) > (CurPlan.ErrorShangXian ))
                    {
                        ResultDictionary["结论"][f] = "不合格";
                        reasonS[f] = "误差超差";
                    }
                    ...
                    {
                        ResultDictionary["结论"][f] = "合格";
                    }


                }else
                {
                    ResultDictionary["误差偏移量(最大误差)"][f] = "-999";
                    ResultDictionary["误差偏移量(最小误差)"][f] = "-999";
                    ResultDictionary["结论"][f] = "不合格";
                    reasonS[f] = "读不到误差";
                }
            ...
            UploadTestResult("最大误差");
            ShowWaitMessage("正在等待{0}秒,请稍候....", 1000 * 1);
            UploadTestResult("最小误差");
            ShowWaitMessage("正在等待{0}秒,请稍候....", 1000 * 1);
            UploadTestResult("误差偏移量(最大误差)");
            ShowWaitMessage("正在等待{0}秒,请稍候....", 1000 * 1);
            UploadTestResult("误差偏移量(最小误差)");        
        ...
        private bool InitEquipment(StPlan_WcPoint _curPoint, float PL, int[] _Pulselap)
        {
            ...
            for (int i = 0; i < BwCount; i++)
            {
                _Pulselap[i] = _curPoint.LapCount;
            }
        ...
        protected override bool CheckPara()
        {
            ...
            GlobalUnit.g_CUS.DnbData.SetWcxPercent(Convert.ToInt32(arrayErrorPara[11]) / 100F, Convert.ToInt32(arrayErrorPara[11]) / 100F);
            //st_Wc.ErrorShangXian = 1;//如果赋值则不再根据规程计算
            //st_Wc.ErrorXiaXian = -1;
            st_Wc.IsCheck = true;
            st_Wc.LapCount = int.Parse(arrayErrorPara[10]);
            GlobalUnit.g_CUS.DnbData.SetCzQsIb(int.Parse(arrayErrorPara[10]), "Ib");
            st_Wc.nCheckOrder = 1;
            st_Wc.Pc = 0;
            st_Wc.PointId = 1;
            ...
            float[] DJ = GetDJ(_Dj);
            if (arrayErrorPara[4].Contains("有"))
            {
                st_Wc.ErrorShangXian = DJ[0];
                st_Wc.ErrorXiaXian = 0 - DJ[0];
            }
            else
            {
                st_Wc.ErrorShangXian = DJ[1];
                st_Wc.ErrorXiaXian = 0 - DJ[1];
            ...
            st_Wc.PowerYinSu = arrayErrorPara[6];
            #region 功率元件
            switch (arrayErrorPara[5])
            {
                case "H":
