...
{
    class DemandCycleSignalOutput:VerifyBase
    {


           #region ----------构造函数----------

        public DemandCycleSignalOutput(object plan)
            : base(plan)
        {
        }
        ...
        {
            ResultNames = new string[] { "标准需量", "实际需量", "需量误差", "周期误差", "结论", "不合格原因" };
            return true;
        }
       ...
       /// <summary>
        ...
        {
            string[] str1 = new string[BwCount];
            int[] iNumber = new int[BwCount];
            bool Result;
            DateTime LastTime = DateTime.Now;
            while (true)
            {
                Result = ReadData(ref str1, ref iNumber, 1F);
                if (Result == true) break;
                DateTime curTime = DateTime.Now;
                int diffTime = CLDC_DataCore.Function.DateTimes.DateDiff(curTime, LastTime);
                if (diffTime > 300)
                {
                    Result = false;
                    break;
            ...
            if (!Result)
            {
                GlobalUnit.g_MsgControl.OutMessage("读取需量周期误差失败！", false, Cus_MessageType.运行时消息);
                Thread.Sleep(200);
            }
            //处理误差
            GlobalUnit.g_MsgControl.OutMessage("开始处理需量周期误差", false);
            MeterBasicInfo curMeter = null;
        
            string[] arrSubKey = new string[BwCount];
            ...
            for (int k = 0; k < BwCount; k++)
            {
                ...
                if (str1[k] != null && str1[k] != "")
                {
                    ...
                    else if (Math.Abs(wc) > 80)
                    {
                        //处理CL188G 误差为-99的问题
                       wc= (float)Math.Round((double)wc / 10000, 3);
                    }
                    str1[k] = wc.ToString();
                ...
                ResultDictionary["周期误差"][k] = str1[k];
              
            }
            MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "周期误差", ResultDictionary["周期误差"]);
        }
