...
namespace CLDC_VerifyAdapter.CostSouth.RecoverParameter
{
    ...
    /// 透支金额限值（恢复）
    /// </summary>
   public class Tzjexz:VerifyBase
    {
        ...
        public override void Verify()
        {
            ...
            string strReaderFileData = CLDC_DataCore.Function.File.ReadInIString(strFile, "Parameter2", "04001003", "");
            ...
            for (int i = 0; i < BwCount; i++)
            {
                if (i <= DataTmp.Length && !string.IsNullOrEmpty(DataTmp[i]))
                {
                    ...
                    strData[i] = "04001003" + DataTmp[i];
                }
            }
            Common.Memset(ref strID, "04001003");
            MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "原始值", strSetData);

            if (Stop) return;
            MessageController.Instance.AddMessage("正在设置透支金额限值,请稍候....");
            result = MeterProtocolAdapter.Instance.SouthParameterElseUpdate(iFlag, strRand2, strData, strID);

            if (Stop) return;
            MessageController.Instance.AddMessage("正在读取透支金额限值,请稍候....");
            string[] strRedData = MeterProtocolAdapter.Instance.ReadData("04001003", 4);
