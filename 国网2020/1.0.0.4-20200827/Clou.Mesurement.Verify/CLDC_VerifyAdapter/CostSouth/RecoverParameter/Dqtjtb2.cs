...
namespace CLDC_VerifyAdapter.CostSouth.RecoverParameter
{
    ...
    /// 当前套第2张阶梯表（恢复）
    /// </summary>
   public class Dqtjtb2:VerifyBase
    {
        ...
        public override void Verify()
        {
            ...
            string strReaderFileData = CLDC_DataCore.Function.File.ReadInIString(strFile, "Price", "040607FF", "");
            ...
            if (Stop) return;
            MessageController.Instance.AddMessage("正在进行设置备用套第2张阶梯表,请稍候....");
            ...
            if (Stop) return;
            MessageController.Instance.AddMessage("正在进行读取当前套第2张阶梯表,请稍候....");
            strRevData = MeterProtocolAdapter.Instance.ReadData("040607FF", 140);
