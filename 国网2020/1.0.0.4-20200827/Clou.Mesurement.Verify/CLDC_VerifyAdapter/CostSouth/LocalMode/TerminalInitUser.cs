...
{
    //交互终端开户
    public class TerminalInitUser : VerifyBase
    {
        ...
        public TerminalInitUser(object plan)
            : base(plan){
        }
        ...
        public override void Verify()
        {
            ...
            try
            {
               ...
               // UploadTestResult("开户前本地开户状态位3");
                ...
                if (Stop) return;
                MessageController.Instance.AddMessage("正在密钥后恢复,请稍候....");
               ...
               string[]  strSyMoneyH4 = MeterProtocolAdapter.Instance.ReadData("00900200", 4);
                ...
                for (int i = 0; i < BwCount; i++)
                {
                    if (Helper.MeterDataHelper.Instance.Meter(i).YaoJianYn)
                    {
                        ...
                        if ( ResultDictionary["第2次开户前本地开户状态位7"][i] == "开户" && ResultDictionary["第2次开户后本地开户状态位7"][i] == "开户" && strRedMeterKhID[i] == strKhID[i]
                ...
                for (int i = 0; i < BwCount; i++)
                {
                    ...
                    if (!string.IsNullOrEmpty(status3[i]))
                    {
                        ...
                        {
                            ResultDictionary["远程开户前本地开户状态位8"][i] = "开户";
                        }
                    }
                    else
                    {
                ...
                if (Stop) return;
                MessageController.Instance.AddMessage("正在读取购电前剩余金额,请稍候....");
                strSyMoneyQ = MeterProtocolAdapter.Instance.ReadData("03330501", 4);
                MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "刷卡前购电次数9", Common.StringConverToDecima(strGdCountQ));
                if (Stop) return;
                MessageController.Instance.AddMessage("正在读取购电前购电次数,请稍候....");
                strGdCountQ = MeterProtocolAdapter.Instance.ReadData("03330201", 2);
                MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "刷卡前剩余金额9", Common.StringConverToDecima(strSyMoneyQ));
                ...
                MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "刷卡后购电次数9", Common.StringConverToIntger(strGdCount));

                if (Stop) return;
                MessageController.Instance.AddMessage("正在读取购电后剩余金额,请稍候....");
                strSyMoney = MeterProtocolAdapter.Instance.ReadData("03330501", 4);
                MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "刷卡后剩余金额9", Common.StringConverToDecima(strSyMoney));

                if (Stop) return;
                MessageController.Instance.AddMessage("正在读取上1次购电后累计购电金额,请稍候....");
                strSyMoneyZLast1 = MeterProtocolAdapter.Instance.ReadData("03330601", 4);
                MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "刷卡后上1次购电后累计购电金额9", Common.StringConverToDecima(strSyMoneyZLast1));
                ...
                for (int i = 0; i < BwCount; i++)
                {
                    ...
                    if (!string.IsNullOrEmpty(strGdCountQ[i]) && !string.IsNullOrEmpty(strSyMoneyQ[i]) && !string.IsNullOrEmpty(strGdCount[i]) && !string.IsNullOrEmpty(strSyMoney[i]))
                        if (strGdCountQ[i] == strGdCount[i] && strSyMoneyQ[i] == strSyMoney[i] && strSyMoneyZLast1[i] == "00030000"
