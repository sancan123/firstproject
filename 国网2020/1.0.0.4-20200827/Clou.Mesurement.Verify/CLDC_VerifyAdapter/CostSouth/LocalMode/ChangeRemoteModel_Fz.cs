...
namespace CLDC_VerifyAdapter.CostSouth.LocalMode
{
    ...
    /// 本地切换远程(辅助功能)
    /// </summary>
    public class ChangeRemoteModel_Fz: VerifyBase
    {
        ...
        public ChangeRemoteModel_Fz(object plan)
            : base(plan)
        {
        }
        ...
        public override void Verify()
        {
            ...
            try
            {
                ...
                for (int i = 0; i < BwCount; i++)
                {
                    ...
                    iSelectBwCount++;
                    if (FkStatusQ[i] == "00") rstTmp[i] = true;
                }
                ...
                if (Array.IndexOf(rstTmp, true) > -1)
                {
                    ...
                    if (Common.GetResultCount(rstTmp, iSelectBwCount / 4))
                    {
                        for (int i = 0; i < BwCount; i++)
                        {
                            ...
                            if (rstTmp[i])
                            {
                                MessageController.Instance.AddMessage("正在对第" + (i + 1) + "块表下发模式切为远程模式换命令,请稍候....");
                    ...
                    {
                        if (Stop) return;
                        MessageController.Instance.AddMessage("下发模式切为远程模式换命令,请稍候....");
                ...
                for (int i = 0; i < BwCount; i++)
                {
                    ...
                    if (ResultDictionary["费控模式(切换后)"][i].Equals("远程"))
                    {
