...
namespace CLDC_VerifyAdapter.CostSouth.LocalMode
{
    ...
    /// 交互终端返写
    /// </summary>
    public class TerminalReplyWrite : VerifyBase
    {
        ...
        public TerminalReplyWrite(object plan)
            : base(plan)
        {
        }
        ...
        public override void Verify()
        {
            ...
            string[] strMoney1 = new string[BwCount];//插卡前剩余金额
            string[] strMoney2 = new string[BwCount];//插卡前剩余金额
            string[] strGdCountH = new string[BwCount];//插卡前购电次数
            ...
            bool[] blnSendTerminalPara = new bool[BwCount];
            bool[] blnSendTerminalPrice2 = new bool[BwCount];
            int iSelectBwCount = 0;
            bool[] bWriteRet = new bool[BwCount];
            string[] strParaInfo = new string[BwCount];
            string[] strPrice1Info = new string[BwCount];
            string[] strPrice2Info = new string[BwCount];
            ...
            result = MeterProtocolAdapter.Instance.SouthParameterUpdate(iFlag, strRand2, strPutApdu, strRevCode, strIBB);
            ...
            result = MeterProtocolAdapter.Instance.SouthParameterUpdate(iFlag, strRand2, strPutApdu, strRevCode, strUBB);
            ...
            result = MeterProtocolAdapter.Instance.SouthMacCheck(iFlag, strRand2, strRevCode, out strAqInfo, out strRevMac1);

            MessageController.Instance.AddMessage("正在读取剩余金额及购电次数,请稍候....");
            Common.Memset(ref strRevCode, "DF01000200000004");
            result = MeterProtocolAdapter.Instance.SouthMacCheck(iFlag, strRand2, strRevCode, out strRevMac1, out strRevMac2, out strGdCount, out strSyMoneyQ);
            #endregion
            ...
            try
            {
                ...
                MessageController.Instance.AddMessage("正式密钥下，通过交互终端使用用户卡本地开户，卡内表号与表内一致,请稍候....");
                for (int i = 0; i < BwCount; i++)
                {
                    ...
                    paraFile[7] = "000001";
                    paraFile[8] = "000001";
                    paraFile[9] = Helper.MeterDataHelper.Instance.Meter(i)._Mb_MeterNo;
                    GdkKhID[i] = strKhID[i];
                    paraFile[10] = GdkKhID[i]; //客户编号
                    paraFile[11] = "01";       //卡类型 01=开户 02=购电 03=补卡

                    strGdMoney[i] = "00001388";
                    GdkGdCount[i] = "00000001";
                ...
                blnSendTerminalPara = MeterProtocolAdapter.Instance.SouthTerminalSendParam(strCardNo, CardType, strParaInfo, out strfileReplyArr);

                ShowWaitMessage("正在等待{0}秒,请稍候....", 1000 * 21);

                if (Stop) return;
                MessageController.Instance.AddMessage("正在写用户卡返写信息文件,请稍候....");
                bWriteRet = MeterProtocolAdapter.Instance.SouthWriteUserCardReply(strfileReplyArr);
                ...
                for (int i = 0; i < BwCount; i++)
                {
                    ...
                    GdkKhID[i] = strKhID[i];
                    paraFile[10] = GdkKhID[i]; //客户编号
                    paraFile[11] = "02";       //卡类型 01=开户 02=购电 03=补卡
                ...
                {
                    if (!Helper.MeterDataHelper.Instance.Meter(i).YaoJianYn) continue;
                    if (strRepReplyDate[i] != "0000000000" && !string.IsNullOrEmpty(strRepReplyDate[i]))
                    {
                        ...
                        if (status3[i] == "0" && strRepSyMoney[i] == "00002710" && bWriteRet[i] && strRepKhID[i] == "112233445566"
                            && strTzMoney[i] == "00000000")
                        {
                            blnRet[i, 0] = true;
                        }
                    }
                ...
                MessageController.Instance.AddMessage("正在复位卡片,请稍候....");
                BlnIniRet = MeterProtocolAdapter.Instance.SouthResetCard();
                MessageController.Instance.AddMessage("通过交互终端使用用户卡充值，卡购电次数＜表购电次数,请稍候....");
                for (int i = 0; i < BwCount; i++)
                {
                    if (!Helper.MeterDataHelper.Instance.Meter(i).YaoJianYn) continue;
                    if (Stop) return;
                ...
                Common.Memset(ref strRevCode, "DF01000200000004");
                result = MeterProtocolAdapter.Instance.SouthMacCheck(iFlag, strRand2, strRevCode, out strRevMac1, out strRevMac2, out strBuyCount, out strSyMoney);
                //if (Stop) return;
                ...
                {
                    if (!Helper.MeterDataHelper.Instance.Meter(i).YaoJianYn) continue;
                    if (strSyMoney[i] == "00002710" && strRepBuyCount[i] == "00000002" && strRepIBB[i] == "000001" && strRepUBB[i] == "000001")
                    {
                        blnRet[i, 1] = true;
                    }
                ...
                MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "卡购电次数2", Common.StringConverToIntger(GdkGdCount));
                ...
                MessageController.Instance.AddMessage("正在复位卡片,请稍候....");
                BlnIniRet = MeterProtocolAdapter.Instance.SouthResetCard();
                MessageController.Instance.AddMessage("通过交互终端使用用户卡充值，卡购电次数=（表购电次数+2），购电金额100元,请稍候....");
                for (int i = 0; i < BwCount; i++)
                {
                    if (!Helper.MeterDataHelper.Instance.Meter(i).YaoJianYn) continue;
                    if (Stop) return;
                ...
                for (int i = 0; i < BwCount; i++)
                {
                    ...
                    {
                        strfileReplyArr[i] = strfileReplyArr[i].Substring(0, strfileReplyArr[i].Length - 6);
                    }
                }
                ...
                {
                    if (!Helper.MeterDataHelper.Instance.Meter(i).YaoJianYn) continue;
                    if (!string.IsNullOrEmpty(strfileReplyArr[i]) && strfileReplyArr[i].Length == 108)
                    {
                ...
                {
                    if (!Helper.MeterDataHelper.Instance.Meter(i).YaoJianYn) continue;
                    if (!string.IsNullOrEmpty(strSyMoney[i]) && !string.IsNullOrEmpty(strRepBuyCount[i]))
                    {
                        if (strSyMoney[i] == "00002710" && strRepBuyCount[i] == "00000002")
                        {
                            blnRet[i, 2] = true;
                        }
                    }
                ...
                MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "卡购电次数3", Common.StringConverToIntger(GdkGdCount));
                ...
                MessageController.Instance.AddMessage("正在复位卡片,请稍候....");
                BlnIniRet = MeterProtocolAdapter.Instance.SouthResetCard();
                MessageController.Instance.AddMessage("通过交互终端使用用户卡充值，卡购电次数=表购电次数，购电金额100元,请稍候....");
                for (int i = 0; i < BwCount; i++)
                {
                    if (!Helper.MeterDataHelper.Instance.Meter(i).YaoJianYn) continue;
                    if (Stop) return;
                ...
                if (Stop) return;
                MessageController.Instance.AddMessage("正在读取卡片备用套电价文件信息,请稍候....");
                result = MeterProtocolAdapter.Instance.SouthReadTerminalToCardPrice2(strCardNo, strRand2, out strPrice2Info);

                if (Stop) return;
                Common.Memset(ref strOpenDate, DateTime.Now.ToString("yyMMddHHmmss"));
                MessageController.Instance.AddMessage("正在模拟交互终端下发备用套电价文件,请稍候....");
                blnSendTerminalPrice2 = MeterProtocolAdapter.Instance.SouthTerminalSendPrice2(strPrice2Info, out strfileReplyArr);
                ...
                result = MeterProtocolAdapter.Instance.SouthMacCheck(iFlag, strRand2, strRevCode, out strpriceFile2Arr, out strRevMac1);
                ...
                for (int i = 0; i < BwCount; i++)
                {
                    ...
                    if (!Helper.MeterDataHelper.Instance.Meter(i).YaoJianYn) continue;
                    if (!string.IsNullOrEmpty(strSyMoney[i]) && strRepReplyDate[i] != "0000000000" && !string.IsNullOrEmpty(strRepReplyDate[i]))
                    {
                        int iDiffErr = DateTimes.DateDiff(DateTimes.FormatStringToDateTime(strOpenDate[i]), DateTimes.FormatStringToDateTime(strRepReplyDate[i]));
                        if (strSyMoney[i] == "00002710" && strSetBydjfl1[i] == strRevBydjfl1[i] && strSetBydjjsr1[i] == strRevBydjjsr1[i]
                            && bWriteRet[i] && iDiffErr < 119)
                        {
                            blnRet[i, 3] = true;
                ...
                //5--------------------
                MessageController.Instance.AddMessage("通过交互终端使用用户卡充值，卡购电次数=表购电次数+1，购电金额100元,请稍候....");
                if (Stop) return;
                Helper.EquipHelper.Instance.RemoteControlOnOrOff(true);
                if (Stop) return;
                MessageController.Instance.AddMessage("正在复位卡片,请稍候....");
                ...
                for (int i = 0; i < BwCount; i++)
                {
                    ...
                    if (!string.IsNullOrEmpty(strfileReplyArr[i]) && !string.IsNullOrEmpty(strpriceFile2Arr[i]) && strfileReplyArr[i].Length >= 100 && strpriceFile2Arr[i].Length >= 398)
                    {
                        ...
                        strRepUBB[i] = strfileReplyArr[i].Substring(10, 6);
                        strRepSyMoney[i] = strfileReplyArr[i].Substring(46, 8);
                        strRepReplyDate[i] = strfileReplyArr[i].Substring(84, 10);
                    }
                }
                ...
                for (int i = 0; i < BwCount; i++)
                {
                    ...
                    {
                        int iDiffErr = DateTimes.DateDiff(DateTimes.FormatStringToDateTime(strOpenDate[i]), DateTimes.FormatStringToDateTime(strRepReplyDate[i]));
                        if (strSyMoney[i] == "00004E20" && strSetByjtz1[i] == strRevByjtz1[i] && strSetByjtdj1[i] == strRevdByjtdj1[i]
                             && bWriteRet[i] && iDiffErr < 120)
                        {
                            blnRet[i, 4] = true;
                ...
                MessageController.Instance.AddMessage("通过交互终端对用户卡2进行补卡,请稍候....");
                for (int i = 0; i < BwCount; i++)
                {
                    if (!Helper.MeterDataHelper.Instance.Meter(i).YaoJianYn) continue;
                    if (Stop) return;
                ...
                for (int i = 0; i < BwCount; i++)
                {
                    ...
                    if (!Helper.MeterDataHelper.Instance.Meter(i).YaoJianYn) continue;
                    string MeterNoTmp = Helper.MeterDataHelper.Instance.Meter(i)._Mb_MeterNo.PadLeft(12, '0');
                    if (!string.IsNullOrEmpty(strRepMeterNo[i]) && strRepReplyDate[i] != "0000000000" && !string.IsNullOrEmpty(strRepReplyDate[i]))
                    {
                        int iDiffErr = DateTimes.DateDiff(DateTimes.FormatStringToDateTime(strOpenDate[i]), DateTimes.FormatStringToDateTime(strRepReplyDate[i]));
                        if (strSyMoney[i] == "00007530" && strRepMeterNo[i] == MeterNoTmp && strRepIBB[i] != strIBB[i] && strRepUBB[i] != strUBB[i] && iDiffErr < 120)
                        {
                            blnRet[i, 5] = true;
                        }
                    }
