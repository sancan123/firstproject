...
namespace CLDC_VerifyAdapter.CostSouth.RemoteMode
{
    ...
    /// 合闸复电（卡片）
    /// </summary>
   public class UserCardRecoverElectricity:VerifyBase
    {
        ...
        public UserCardRecoverElectricity(object plan)
            : base(plan)
        {
        }
       //继电器命令状态位测试前一测试后|合闸信号输出|插卡时间|上1次合闸记录|合闸次数前一后|合闸复电(卡片)

        protected override bool CheckPara()
        {
            //return base.CheckPara();
            ResultNames = new string[] { "继电器状态位测试前一测试后", "表内电流前一后（内置）",
                                         "合闸信号输出", "插卡时间", "上1次合闸记录", "合闸次数前一后",
                                         "合闸复电（卡片）", "结论" }; 
            return true;
        }
        ...
        public override void Verify()
        {
            ...
            MessageController.Instance.AddMessage("正在通过用户卡执行本地合闸,请稍候....");
            if (Stop) return;
            ...
            if (GlobalUnit.IsNZLoadRelayControl && GlobalUnit.IsDan)
            {
                ...
                //继电器命令状态位测试前-测试后|合闸信号输出|插卡时间|上1次合闸记录|合闸次数前一后|合闸复电(卡片)
                if (Stop) return;
                ...
                MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "插卡时间", strData);
                ...
                for (int i = 0; i < BwCount; i++)
                {
                    ...
                    ResultDictionary["合闸复电（卡片）"][i] = "不通过";
                    ...
                    {
                        string dateTmp = strHzsjInfo[i].Substring(strHzsjInfo[i].Length - 12, 12);
                        if (dateTmp != "000000000000")
                        {
                            int iErr = DateTimes.DateDiff(DateTimes.FormatStringToDateTime(strHzDate), DateTimes.FormatStringToDateTime(dateTmp));
                            if (blnRet[i, 0] && iErr < 600 && Convert.ToInt32(iHeZhaCount3H[i]) == Convert.ToInt32(iHeZhaCount3Q[i]) + 1
                                && strMeterIbQ[i] == "0" && strMeterIbH[i] != "0")
                            {
                                ResultDictionary["合闸复电（卡片）"][i] = "通过";
                                ResultDictionary["结论"][i] = "合格";
                            }
                        }
                    }
                }
                UploadTestResult("合闸复电（卡片）");
                UploadTestResult("结论");
                #endregion
            }
            else
            {
                ...
                for (int i = 0; i < BwCount; i++)
                {
                    ...
                    ResultDictionary["合闸复电（卡片）"][i] = ResultDictionary["合闸信号输出"][i].Equals("是") ? "成功" : "失败";
                    ResultDictionary["合闸次数前一后"][i] = iHeZhaCount3Q[i].ToString() + "-" + iHeZhaCount3H[i].ToString();
                }
                UploadTestResult("合闸信号输出");
                UploadTestResult("合闸次数前一后");
                ...
                for (int i = 0; i < BwCount; i++)
                {
                    ...
                    if (!string.IsNullOrEmpty(strHzsjInfo[i]) && strHzsjInfo[i].Length >= 68 && !string.IsNullOrEmpty(iHeZhaCount3H[i].ToString()) && !string.IsNullOrEmpty(iHeZhaCount3Q[i].ToString()))
                    {
                        ...
                        {
                            int iErr = DateTimes.DateDiff(DateTimes.FormatStringToDateTime(strHzDate), DateTimes.FormatStringToDateTime(dateTmp));
                            if (blnRet[i, 0] && blnRet[i, 1] && blnRet[i, 2] && iErr < 180 && Convert.ToInt32(iHeZhaCount3H[i]) == Convert.ToInt32(iHeZhaCount3Q[i]) + 1)
                            {
                                ResultDictionary["结论"][i] = "合格";
                            }
                        }
