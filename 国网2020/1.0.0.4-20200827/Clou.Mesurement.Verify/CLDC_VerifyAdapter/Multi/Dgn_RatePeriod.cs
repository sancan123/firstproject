...
namespace CLDC_VerifyAdapter.Multi
{
    ...
    /// </summary>
    class Dgn_RatePeriod : DgnBase
    {
        public Dgn_RatePeriod(object plan) : base(plan)
            ...
            for (int k = 0; k < arrSD.Length; k++)
            {
                ...
                */
                ...
                for (int iNum = 0; iNum < BwCount; iNum++)
                {
                    ...
                    ResultDictionary["误差"][iNum] = ResultDictionary["误差"][iNum] + "|" + Math.Abs(fErr).ToString() + "(" + curTri.ToString() + ")";
