...
{
    /// <summary>
    /// 电压不平衡事件记录，模拟触发10次，读取1,4，7，10
    /// </summary>
    class EL_VoltageImbalance:EventLogBase
    {
        public EL_VoltageImbalance(object plan)
            : base(plan) 
        {
        ...
        public override void Verify()
        {
            ...
            if (Stop) return;
            MessageController.Instance.AddMessage("正在读取电压不平衡事件产生前电流不平衡总次数");
           ...
           for (int it = 0; it < 1; it++)
           {
               ...
               MessageController.Instance.AddMessage("正在进行第" + (it + 1) + "次让电能表电压不平衡以便形成失压记录...");

               CLDC_VerifyAdapter.Helper.EquipHelper.Instance.PowerOn(GlobalUnit.U * 0.025F, GlobalUnit.U, GlobalUnit.U, 0, 0, 0, (int)Cus_PowerYuanJian.H, 50, "1.0", false, false, false, (int)Cus_PowerFangXiang.正向有功);
               ShowWaitMessage("正在等待{0}秒,请稍候....", 1000 * 65);
               CLDC_VerifyAdapter.Helper.EquipHelper.Instance.PowerOn(GlobalUnit.U, GlobalUnit.U, GlobalUnit.U, 0, 0, 0, (int)Cus_PowerYuanJian.H, 50, "1.0", true, false, false, (int)CLDC_Comm.Enum.Cus_PowerFangXiang.正向有功);

               ShowWaitMessage("正在等待{0}秒,请稍候....", 1000 * 62);
           }

            if (Stop) return;
            MessageController.Instance.AddMessage("正在读取电压不平衡事件产生后电流不平衡总次数");
            ...
            string[] strLoseTimeQ = MeterProtocolAdapter.Instance.ReadData("16000101", 6);
            ...
            string[] strLoseTimeQ4 = MeterProtocolAdapter.Instance.ReadData("17000104", 6);
            ...
            string[] strLoseTimeQ7 = MeterProtocolAdapter.Instance.ReadData("16000107", 6);
            ...
            string[] strLoseTimeQ10 = MeterProtocolAdapter.Instance.ReadData("1600010A", 6);
