...
{
    /// <summary>
    /// 时段表编程事件，模拟触发10次，读取1,4，7，10
    /// </summary>
    class EL_TimeProgram : EventLogBase
    {

        public EL_TimeProgram(object plan)
            : base(plan) 
        {
        ...
        public override void Verify()
        {
            ...
            if (Stop) return;
            MessageController.Instance.AddMessage("正在读取电表时段表编程事件总次数");
            string[] strLoseCountQ = MeterProtocolAdapter.Instance.ReadData("03BE0000", 3);
            ...
            {
                if (Stop) return;
                MessageController.Instance.AddMessage("正在让电能表时段表编程以便形成编程记录");
                //  DateTime time = DateTime.Now
              
                if (Stop) return;
                MessageController.Instance.AddMessage("正在设置第一套第1日时段表,请稍候....");
               // Common.Memset(ref iFlag, 1);
                Common.Memset(ref strID, "04020001");
            ...
            if (Stop) return;
            MessageController.Instance.AddMessage("正在读取电表编程记录产生后时段表编程记录总次数");
            string[] strLoseCountH = MeterProtocolAdapter.Instance.ReadData("03BE0000", 3);
            ...
            string[] strLoseTimeQ = MeterProtocolAdapter.Instance.ReadData("03BE0001", 6);
            ...
            string[] strLoseTimeQ4 = MeterProtocolAdapter.Instance.ReadData("03BE0004", 6);
            ...
            string[] strLoseTimeQ7 = MeterProtocolAdapter.Instance.ReadData("03BE0007", 6);
            ...
            string[] strLoseTimeQ10 = MeterProtocolAdapter.Instance.ReadData("03BE000A", 6);
