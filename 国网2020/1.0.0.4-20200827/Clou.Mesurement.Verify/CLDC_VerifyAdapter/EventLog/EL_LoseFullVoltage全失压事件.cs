...
{
    /// <summary>
    /// 全失压事件记录,，模拟触发10次，读取1,4，7，10
    /// </summary>
    class EL_LoseFullVoltage : EventLogBase
    {
               
        //全失压记录

        public EL_LoseFullVoltage(object plan)
            : base(plan) 
        {
        ...
        public override void Verify()
        {
            ...
            if (Stop) return;
            MessageController.Instance.AddMessage("正在读取电表全失压记录总次数");
            string[] strLoseCountQ = MeterProtocolAdapter.Instance.ReadData("03050000", 3);
            ...
            if (Stop) return;
            MessageController.Instance.AddMessage("正在让电能表全失压以便形成全失压记录");

                //设置10次校时
            for (int it = 0; it < 10; it++)
            {
                MessageController.Instance.AddMessage("正模拟第" +(it+1)+ "次全失压");
                 //MeterProtocolAdapter.Instance.WriteDateTime(DateTime.Now.AddMinutes(it).ToString("yyMMddHHmmss"));
                 CLDC_VerifyAdapter.Helper.EquipHelper.Instance.PowerOn(GlobalUnit.U * 0.55F, GlobalUnit.U * 0.55F, GlobalUnit.U * 0.55F, GlobalUnit.Ib * 0.06F, GlobalUnit.Itr , GlobalUnit.Itr , (int)Cus_PowerYuanJian.H, 50, "1.0", false, false, false, (int)CLDC_Comm.Enum.Cus_PowerFangXiang.正向有功);
            ...
            if (Stop) return;
            MessageController.Instance.AddMessage("正在读取电表全失压记录产生后全失压记录总次数");
            string[] strLoseCountH = MeterProtocolAdapter.Instance.ReadData("03050000", 3);
            MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "事件产生后事件次数", strLoseCountH);
            //上1次
            if (Stop) return;
            MessageController.Instance.AddMessage("正在读取上1次电表全失压记录发生时刻");
            string[] strLoseTimeQ = MeterProtocolAdapter.Instance.ReadData("03050001", 6);
            MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "上1次事件记录发生时刻", strLoseTimeQ);

            //上4次
            if (Stop) return;
            MessageController.Instance.AddMessage("正在读取上4次电表全失压记录发生时刻");
            string[] strLoseTimeQ4 = MeterProtocolAdapter.Instance.ReadData("03050004", 6);
            MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "上4次事件记录发生时刻", strLoseTimeQ4);
            //上7次
            if (Stop) return;
            MessageController.Instance.AddMessage("正在读取上7次电表全失压记录发生时刻");
            string[] strLoseTimeQ7 = MeterProtocolAdapter.Instance.ReadData("03050007", 6);
            MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "上7次事件记录发生时刻", strLoseTimeQ7);
            //上10次
            if (Stop) return;
            MessageController.Instance.AddMessage("正在读取上10次电表全失压记录发生时刻");
            string[] strLoseTimeQ10 = MeterProtocolAdapter.Instance.ReadData("0305000A", 6);
