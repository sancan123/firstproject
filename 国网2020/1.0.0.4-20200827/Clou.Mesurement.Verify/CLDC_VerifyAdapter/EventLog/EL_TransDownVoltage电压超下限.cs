...
namespace CLDC_VerifyAdapter.EventLog
{
    ...
    /// 电压下限
    /// </summary>
    class EL_TransDownVoltage:EventLogBase
    {
        public EL_TransDownVoltage(object plan)
            : base(plan) 
        {
        ...
        public override void Verify()
        {
            ...
            string[] strLoseCountQ = MeterProtocolAdapter.Instance.ReadData("03570000", 3);
            MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "事件产生前事件次数", strLoseCountQ);

            if (Stop) return;

            CLDC_VerifyAdapter.Helper.EquipHelper.Instance.PowerOn(GlobalUnit.U * 0.89f, GlobalUnit.U * 0.89f, GlobalUnit.U * 0.89f, 0, 0, 0, (int)Cus_PowerYuanJian.H, 50, "1.0", true, false, false, (int)Cus_PowerFangXiang.正向有功);
            ...
            //上1次次发生时刻记录内容
            if (Stop) return;
            MessageController.Instance.AddMessage("正在读取电压超下限事件产生后电压超上限总次数");
            string[] strLoseCountH = MeterProtocolAdapter.Instance.ReadData("03570000", 3);
            MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "事件产生后事件次数", strLoseCountH);

            if (Stop) return;
            MessageController.Instance.AddMessage("正在读取上1次电压超下限发生时刻");
            string[] strLoseTimeQ = MeterProtocolAdapter.Instance.ReadData("03570001", 12);
            ...
            //上4次次发生时刻记录内容
            if (Stop) return;
            MessageController.Instance.AddMessage("正在读取上4次电压超下限发生时刻");
            string[] strLoseTimeQ4 = MeterProtocolAdapter.Instance.ReadData("03570004", 12);

            MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "上4次事件记录发生时刻", strLoseTimeQ4);

            //上7次次发生时刻记录内容
            if (Stop) return;
            MessageController.Instance.AddMessage("正在读取上7次电压超下限发生时刻");
            string[] strLoseTimeQ7 = MeterProtocolAdapter.Instance.ReadData("03570007", 12);
            ...
            //上10次次发生时刻记录内容
            if (Stop) return;
            MessageController.Instance.AddMessage("正在读取上10次电压超下限发生时刻");
            string[] strLoseTimeQ10 = MeterProtocolAdapter.Instance.ReadData("0357000A", 12);
