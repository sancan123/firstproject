...
using System; using CLDC_DataCore;
...
namespace CLDC_VerifyAdapter.EventLog
{
    ...
    /// 需量清零事件
    /// </summary>
    class EL_ClearDemand : EventLogBase
    {
        public EL_ClearDemand(object plan)
            : base(plan) 
        {
        ...
        public override void Verify()
        {
            ...
            if (Stop) return;
            MessageController.Instance.AddMessage("正在读取清需量事件产生前清需量总次数");
            string[] strLoseCountQ = MeterProtocolAdapter.Instance.ReadData("03300200", 3);
            MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "事件产生前事件次数", strLoseCountQ);

            if (Stop) return;
            MessageController.Instance.AddMessage("正在进行清需量...");
            MeterProtocolAdapter.Instance.ClearDemand();
            ShowWaitMessage("正在等待{0}秒,请稍候....", 1000 * 10);

            if (Stop) return;
            MessageController.Instance.AddMessage("正在读取清需量事件产生后清需量总次数");
            string[] strLoseCountH = MeterProtocolAdapter.Instance.ReadData("03300200", 3);
            MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "事件产生后事件次数", strLoseCountH);

            //  //上1次发生时刻记录内容
            if (Stop) return;
            MessageController.Instance.AddMessage("正在读取上1次清需量发生时刻");
            string[] strLoseTimeQ = MeterProtocolAdapter.Instance.ReadData("03300201", 6);
