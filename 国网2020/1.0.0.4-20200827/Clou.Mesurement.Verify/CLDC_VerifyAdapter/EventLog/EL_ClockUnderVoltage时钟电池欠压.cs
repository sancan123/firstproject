...
namespace CLDC_VerifyAdapter.EventLog
{
    ...
    /// 时钟电池欠压
    /// </summary>
    class EL_ClockUnderVoltage : EventLogBase
    {
        public EL_ClockUnderVoltage(object plan)
            : base(plan) 
        {
        ...
        public override void Verify()
        {
            ...
            if (Stop) return;
            MessageController.Instance.AddMessage("正在读取电表时钟电池欠压记录总次数");
            string[] strLoseCountQ = MeterProtocolAdapter.Instance.ReadData("22000000", 3);
            MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "事件产生前事件次数", strLoseCountQ);

            if (Stop) return;
            MessageController.Instance.AddMessage("正在让电能表时钟电池欠压以便形成欠压记录");
            System.Windows.Forms.MessageBox.Show("请手动拔出电能表时钟电池后点击确定。");
            ...
            string[] strLoseCountH = MeterProtocolAdapter.Instance.ReadData("22000000", 3);
            ...
            string[] strLoseTimeQ = MeterProtocolAdapter.Instance.ReadData("22000001", 6);
