...
{
    /// <summary>
    /// 过流事件记录，模拟触发10次，读取1,4，7，10
    /// </summary>
    class EL_OverCurrent : EventLogBase
    {
       

        public EL_OverCurrent(object plan)
            : base(plan) 
        {
        ...
        public override void Verify()
        {
            ...
            string[] strVoltageUp = MeterProtocolAdapter.Instance.ReadData("04090801", 2);
         
            iFlag = MeterProtocolAdapter.Instance.SouthCheckIdentity(out strRand1, out strRand2, out strEsamNo);
            Common.Memset(ref strDataCode, "04090801");
            Common.Memset(ref strData, "04090801" + (GlobalUnit.Imax).ToString().Replace(".", "").PadLeft(4, '0'));
            MessageController.Instance.AddMessage("正在设置阀值04090801为：8");
            bool[] bResult = MeterProtocolAdapter.Instance.SouthParameterElseUpdate(iFlag, strRand2, strData, strDataCode);
            if (Stop) return;
            MessageController.Instance.AddMessage("正在读取电表过流记录总次数");
            string[] strLoseCountQ = MeterProtocolAdapter.Instance.ReadData("19010001", 3);
            ...
            {
                if (Stop) return;
                string strDt = DateTime.Now.AddHours(it + 0).ToString("yyMMddHHmmss");
               // MeterProtocolAdapter.Instance.WriteDateTime(strDt);
                MessageController.Instance.AddMessage("正在第" + (it + 1) + "让电能表过流以便形成过流记录");
                CLDC_VerifyAdapter.Helper.EquipHelper.Instance.PowerOn(GlobalUnit.U, GlobalUnit.U, GlobalUnit.U, GlobalUnit.Imax * 0.15f, GlobalUnit.Imax * 0.15f, GlobalUnit.Imax * 0.15f, (int)Cus_PowerYuanJian.H, 50, "1.0", false, false, false, (int)CLDC_Comm.Enum.Cus_PowerFangXiang.正向有功);
            ...
            if (Stop) return;
            MessageController.Instance.AddMessage("正在读取电表过流记录产生后过流记录总次数");
            string[] strLoseCountH = MeterProtocolAdapter.Instance.ReadData("19010001", 3);
            MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "事件产生后事件次数", strLoseCountH);
            
            //上1次次发生时刻记录内容
            if (Stop) return;
            MessageController.Instance.AddMessage("正在读取上1次电表过流记录发生时刻");
            string[] strLoseTimeQ = MeterProtocolAdapter.Instance.ReadData("19010101", 6);
            ...
            string[] strLoseTimeQ4 = MeterProtocolAdapter.Instance.ReadData("19010104", 6);
            ...
            string[] strLoseTimeQ7 = MeterProtocolAdapter.Instance.ReadData("19010107", 6);
            ...
            string[] strLoseTimeQ10 = MeterProtocolAdapter.Instance.ReadData("1901010A", 6);
            MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "上10次事件记录发生时刻", strLoseTimeQ10);

            //if (Stop) return;
            //MessageController.Instance.AddMessage("正在读取上1次电表过流记录结束时刻");
            //string[] strLoseTimeH = MeterProtocolAdapter.Instance.ReadData("19010101", 6);
            ...
            MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "不合格原因", reasonS);
            //恢复阀值
            Common.Memset(ref strData, "04090801" +( GlobalUnit.Imax*10).ToString().Replace(".","").PadLeft(4,'0'));
            MessageController.Instance.AddMessage("正在恢复阀值04090801为：" + GlobalUnit.Imax);
