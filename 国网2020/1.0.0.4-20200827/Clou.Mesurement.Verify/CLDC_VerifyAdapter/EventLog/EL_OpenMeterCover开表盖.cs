...
namespace CLDC_VerifyAdapter.EventLog
{
    ...
    /// 开表盖事件记录
    /// </summary>
    class EL_OpenMeterCover:EventLogBase
    {
        public EL_OpenMeterCover(object plan)
            : base(plan) 
        {
        ...
        public override void Verify()
        {
            ...
            if (Stop) return;
            MessageController.Instance.AddMessage("正在读取电表开表盖记录总次数");
            string[] strLoseCountQ = MeterProtocolAdapter.Instance.ReadData("03300D00", 3);
            ...
            {
                if (Stop) return;
                MessageController.Instance.AddMessage("正在让电能表打开表盖以便形成开盖记录");
                System.Windows.Forms.MessageBox.Show("请手动打出电能表盖1次后点击确定。");
                ShowWaitMessage("正在等待{0}秒,请稍候....", 1000 * 65);
                System.Windows.Forms.MessageBox.Show("请手动装回电能表盖后点击确定。");

            }
            if (Stop) return;
            MessageController.Instance.AddMessage("正在读取电表开表盖后产生开表盖记录总次数");
            string[] strLoseCountH = MeterProtocolAdapter.Instance.ReadData("03300D00", 3);
            MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "事件产生后事件次数", strLoseCountH);
            //上一次
            if (Stop) return;
            MessageController.Instance.AddMessage("正在读取上1次电表开表盖记录发生时刻");
            string[] strLoseTimeQ = MeterProtocolAdapter.Instance.ReadData("03300D01", 6);
            MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "上1次事件记录发生时刻", strLoseTimeQ);
            ////上4次
            //if (Stop) return;
            //MessageController.Instance.AddMessage("正在读取上4次电表开表盖记录发生时刻");
            //string[] strLoseTimeQ4 = MeterProtocolAdapter.Instance.ReadData("03300D04", 6);
            //MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "上4次事件记录发生时刻", strLoseTimeQ4);
            ////上7次
            //if (Stop) return;
            //MessageController.Instance.AddMessage("正在读取上7次电表开表盖记录发生时刻");
            //string[] strLoseTimeQ7 = MeterProtocolAdapter.Instance.ReadData("03300D07", 6);
            //MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "上7次事件记录发生时刻", strLoseTimeQ7);
            ////上10次
            //if (Stop) return;
            //MessageController.Instance.AddMessage("正在读取上10次电表开表盖记录发生时刻");
            //string[] strLoseTimeQ10 = MeterProtocolAdapter.Instance.ReadData("03300D0A", 6);
