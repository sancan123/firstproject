...
namespace CLDC_VerifyAdapter.EventLog
{
    ...
    /// 计量事件上报和设置
    /// </summary>
    class EL_EventUploadSetJ : EventLogBase
    {
        public EL_EventUploadSetJ(object plan)
            : base(plan)
        {
        ...
        public override void Verify()
        {
            ...
            //bResult = MeterProtocolAdapter.Instance.WriteData("04001503", 12, CLDC_VerifyAdapter.Helper.EquipHelper.Instance.RevString("B7FB7E007E007E00FF07EEFF", 12));
            ...
            Common.Memset(ref strData, "04001104" + "0010000000000000");//电表运行特征字 1  设置校时为开
            bResult = MeterProtocolAdapter.Instance.SouthParameterElseUpdate(iFlag, strRand2, strData, strDataCode);
          ...
          //  bResult = MeterProtocolAdapter.Instance.WriteData("04001503", 12, CLDC_VerifyAdapter.Helper.EquipHelper.Instance.RevString("B7FB7E007E007E00FF07EEFF", 12));
            ...
            for (int i = 0; i < BwCount; i++)
            {
                ...
                {
                    string strOpenTime = strStatusOpen1[i].Substring(0, 2);//校时次数
                    string strOpenSB = strStatusOpen1[i].Substring(6, 2);//校时上报状态字


                    string strOpenResetTime = strStatusOpenReset[i].Substring(0, 2);//校时次数
                    string strOpenResetSB = strStatusOpenReset[i].Substring(6, 2);//校时上报状态字

                    string strOpenCloseTime = strStatusClose[i].Substring(0, 2);//校时次数
                    string strOpenCloseSB = strStatusClose[i].Substring(6, 2);//校时上报状态字
                    //打开校时状态
                    if (strOpenTime != "01")
                    {
                        ResultDictionary["结论"][i] = Variable.CTG_BuHeGe;
                        reasonS[i] = "校时状态字打开情况下，上报状态字次数不对，应该为1;";
                        continue;
                    }
                    if (strOpenSB != "10")
                    {
                        ResultDictionary["结论"][i] = Variable.CTG_BuHeGe;
                        reasonS[i] = "校时状态字打开情况下，上报状态字不对，应该为10;";
                        continue;
                    }
                    //打开校时，复位后状态
                    ...
                    {
                        ResultDictionary["结论"][i] = Variable.CTG_BuHeGe;
                        reasonS[i] = "校时状态字复位情况下，上报状态字次数不对;";
                        continue;
                    }
                    if (strOpenResetSB != "00")
                    {
                        ResultDictionary["结论"][i] = Variable.CTG_BuHeGe;
                        reasonS[i] = "校时状态字复位情况下，上报状态字不对，应该为00;";
                        continue;
                    }
                    //关校时状态
                    ...
                    {
                        ResultDictionary["结论"][i] = Variable.CTG_BuHeGe;
                        reasonS[i] = "校时状态字关情况下，上报状态字次数不对，应该为1;";
                        continue;
                    }
                    if (strOpenCloseSB != "00")
                    {
                        ResultDictionary["结论"][i] = Variable.CTG_BuHeGe;
                        reasonS[i] = "校时状态字关情况下，上报状态字不对，应该为00;";
                        continue;
                    }
