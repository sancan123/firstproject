...
{
    /// <summary>
    /// 电流不平衡事件记录，模拟触发10次，读取1,4，7，10
    /// </summary>
    class EL_CurrentImbalance:EventLogBase
    {
        public EL_CurrentImbalance(object plan)
            : base(plan) 
        {
        ...
        public override void Verify()
        {
            ...
            if (Stop) return;
            MessageController.Instance.AddMessage("正在读取电流不平衡事件产生前电流不平衡总次数");
            string[] strLoseCountQ = MeterProtocolAdapter.Instance.ReadData("17000001", 3);
           ...
           for (int it = 0; it < 1; it++)
           {
               ...
               MessageController.Instance.AddMessage("正在进行第" + (it + 1) + "次让电能表电流不平衡以便形成失压记录..."); 
            ...
            if (Stop) return;
            MessageController.Instance.AddMessage("正在读取电流不平衡事件产生后电流不平衡总次数");
            string[] strLoseCountH = MeterProtocolAdapter.Instance.ReadData("17000001", 3);
            ...
            string[] strLoseTimeQ = MeterProtocolAdapter.Instance.ReadData("17000101", 6);
            ...
            string[] strLoseTimeQ7 = MeterProtocolAdapter.Instance.ReadData("17000107", 6);
            ...
            string[] strLoseTimeQ10 = MeterProtocolAdapter.Instance.ReadData("1700010A", 6);
