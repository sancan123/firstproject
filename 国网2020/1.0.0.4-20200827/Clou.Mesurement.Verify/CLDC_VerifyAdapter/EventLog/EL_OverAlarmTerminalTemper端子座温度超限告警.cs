...
namespace CLDC_VerifyAdapter.EventLog
{
    ...
    /// 端子座温度超限告警
    /// </summary>
    class EL_OverAlarmTerminalTemper : EventLogBase
    {
        ...
        public EL_OverAlarmTerminalTemper(object plan)
            : base(plan)
        {
        ...
        private void PositionTest(Object obj)
        {
            int Index = (int)obj;
            int TestDelay = 0;
            float AlarmCount = MeterProtocolAdapter.Instance.ReadData("03580000", 3, 0, Index);
            ...
            while (!Stop && TestDelay > 0)
            {
                ...
                float tmpAlarmCount = MeterProtocolAdapter.Instance.ReadData("03580000", 3, 0, Index);
            ...
            if (Result[Index])
            {
                ...
                while (!Stop && TestDelay > 0)
                {
                    ...
                    if (tmpAlarmCount > AlarmCount)
                    {
                        int AlarmFlag_0 = Convert.ToInt32(MeterProtocolAdapter.Instance.ReadDataByPos("04000504", 2, Index), 16);
                        int AlarmFlag_1 = Convert.ToInt32(MeterProtocolAdapter.Instance.ReadDataByPos("04000505", 2, Index), 16);
                        int AlarmFlag_2 = Convert.ToInt32(MeterProtocolAdapter.Instance.ReadDataByPos("04000506", 2, Index), 16);
                        Result[Index] = false;
                        Result[Index] |= (AlarmFlag_0 & 0x0800) == 0x0800;
                        Result[Index] |= (AlarmFlag_1 & 0x0800) == 0x0800;
                        Result[Index] |= (AlarmFlag_2 & 0x0800) == 0x0800;
                        if (Result[Index])
                        {
                            reasonS[Index] = "";
                        }
        ...
        public override void Verify()
        {
            ...
            m_AlarmTemperature = MeterProtocolAdapter.Instance.ReadData("04091201", 2, 1);
            m_ResumeTemperature = MeterProtocolAdapter.Instance.ReadData("04091202", 2, 1);
            m_DelayTime = MeterProtocolAdapter.Instance.ReadData("04091203", 1, 0);
            ...
            string[] strLoseCountQ = MeterProtocolAdapter.Instance.ReadData("03580000", 3);
            MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "事件产生前事件次数", strLoseCountQ);
            ...
            string[] strLoseCountH = MeterProtocolAdapter.Instance.ReadData("03580000", 3);
            ...
            string[] strLoseTimeQ = MeterProtocolAdapter.Instance.ReadData("03580001", 6);
