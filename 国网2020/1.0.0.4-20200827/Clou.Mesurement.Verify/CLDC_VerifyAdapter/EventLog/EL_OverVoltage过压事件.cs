...
{
    /// <summary>
    /// 过压事件记录，模拟触发10次，读取1,4，7，10
    /// </summary>
    class EL_OverVoltage : EventLogBase
    {




        public EL_OverVoltage(object plan)
            : base(plan) 
        {
        ...
        public override void Verify()
        {
            ...
            if (Stop) return;
            MessageController.Instance.AddMessage("正在读取电表过压记录总次数");
            string[] strLoseCountQ = MeterProtocolAdapter.Instance.ReadData("12010001", 3);
            ...
            for (int it = 0; it < 10; it++)
            {
                ...
                MessageController.Instance.AddMessage("正在第" + (it + 1) + "让电能表过流以便形成过压记录");
                CLDC_VerifyAdapter.Helper.EquipHelper.Instance.PowerOn(GlobalUnit.U * 1.3f, GlobalUnit.U, GlobalUnit.U, GlobalUnit.Ib, GlobalUnit.Ib, GlobalUnit.Ib, (int)Cus_PowerYuanJian.H, 50, "1.0", false, false, false, (int)CLDC_Comm.Enum.Cus_PowerFangXiang.正向有功);
            ...
            if (Stop) return;
            MessageController.Instance.AddMessage("正在读取电表过压记录产生后过压记录总次数");
            string[] strLoseCountH = MeterProtocolAdapter.Instance.ReadData("12010001", 3);
            MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "事件产生后事件次数", strLoseCountH);
            //上1次次发生时刻记录内容
            if (Stop) return;
            MessageController.Instance.AddMessage("正在读取上1次电表过压记录发生时刻");
            string[] strLoseTimeQ = MeterProtocolAdapter.Instance.ReadData("12010101", 6);
            MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "事件记录发生时刻", strLoseTimeQ);

            //上4次发生时刻记录内容
            if (Stop) return;
            MessageController.Instance.AddMessage("正在读取上4次电表过压记录发生时刻");
            string[] strLoseTimeQ4 = MeterProtocolAdapter.Instance.ReadData("12010104", 6);
            MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "上4次事件记录发生时刻", strLoseTimeQ4);

            //上7次发生时刻记录内容
            if (Stop) return;
            MessageController.Instance.AddMessage("正在读取上7次电表过压记录发生时刻");
            string[] strLoseTimeQ7 = MeterProtocolAdapter.Instance.ReadData("12010107", 6);
            MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "上7次事件记录发生时刻", strLoseTimeQ7);


            //上10次发生时刻记录内容
            if (Stop) return;
            MessageController.Instance.AddMessage("正在读取上10次电表过压记录发生时刻");
            string[] strLoseTimeQ10 = MeterProtocolAdapter.Instance.ReadData("1201010A", 6);
