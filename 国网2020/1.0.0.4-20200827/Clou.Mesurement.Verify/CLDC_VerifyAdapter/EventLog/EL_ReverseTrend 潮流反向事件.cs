...
namespace CLDC_VerifyAdapter.EventLog
{
    ...
    /// 潮流反向事件记录
    /// </summary>
    class EL_ReverseTrend : EventLogBase
    {
    
        public EL_ReverseTrend(object plan)
            : base(plan) 
        {
        ...
        public override void Verify()
        {
            ...
            if (Stop) return;
            MessageController.Instance.AddMessage("正在读取电表潮流反向记录总次数");
            string[] strLoseCountQ = MeterProtocolAdapter.Instance.ReadData("1B010001", 3);
            ...
            for (int it = 0; it < 1; it++)
            {
                ...
                MessageController.Instance.AddMessage("正在进行第" + (it + 1) + "次让电能表潮流反向以便形成失压记录...");


                CLDC_VerifyAdapter.Helper.EquipHelper.Instance.PowerOn(GlobalUnit.U, GlobalUnit.U, GlobalUnit.U, GlobalUnit.Itr, GlobalUnit.Itr, GlobalUnit.Itr, (int)Cus_PowerYuanJian.H, 50, "-0.9999", false, false, true, (int)CLDC_Comm.Enum.Cus_PowerFangXiang.正向有功);
            ...
            if (Stop) return;
          //  MessageController.Instance.AddMessage("正在让电能表潮流反向以便形成潮流反向记录");
         ...
         //   Helper.EquipHelper.Instance.DirectRemoteControl();
          ...
          //  ShowWaitMessage("正在等待{0}秒,请稍候....", 1000 * 100);
            ...
            if (Stop) return;
            MessageController.Instance.AddMessage("正在读取电表潮流反向记录产生后潮流反向记录总次数");
            string[] strLoseCountH = MeterProtocolAdapter.Instance.ReadData("1B010001", 3);
            MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "事件产生后事件次数", strLoseCountH);

            //上1次发生时刻记录内容
            if (Stop) return;
            MessageController.Instance.AddMessage("正在读取上1次电压不平衡发生时刻");
            string[] strLoseTimeQ = MeterProtocolAdapter.Instance.ReadData("1B010101", 6);
            ...
            string[] strLoseTimeQ4 = MeterProtocolAdapter.Instance.ReadData("1B010401", 6);
            ...
            string[] strLoseTimeQ7 = MeterProtocolAdapter.Instance.ReadData("1B010701", 6);
            ...
            string[] strLoseTimeQ10 = MeterProtocolAdapter.Instance.ReadData("1B010A01", 6);
