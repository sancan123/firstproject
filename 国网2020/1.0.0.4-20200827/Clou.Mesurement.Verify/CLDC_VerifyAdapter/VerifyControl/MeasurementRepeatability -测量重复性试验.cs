...
namespace CLDC_VerifyAdapter
{
    ...
    /// 测量重复性试验
    /// </summary>
    class MeasurementRepeatability : VerifyBase
    {
        #region ----------变量声明----------
        ...
        #region ----------构造函数----------
        public MeasurementRepeatability(object plan)
            : base(plan)
        {
            //检测检定参数是否正确
            // m_VerifyDemo.BWCount = BwCount;
            // ResultNames = new string[] { "功率元件", "功率方向", "电流倍数", "功率因数", "误差下限", "误差上限", "误差圈数", "误差1", "误差2", "误差3", "误差4", "误差5", "标准偏差估计值", "结论" };
            ResultNames = new string[] { "误差1", "误差2", "误差3", "误差4", "误差5", "误差6", "误差7", "误差8", "误差9", "误差10", "偏差值", "结论" };
        ...
        public override void Verify()
        {
            ...
            int tableHeader = 5;// curPoint.Pc == 0 ? m_WCTimesPerPoint : m_WindageTimesPerPoint;  //每个点合格误差次数
            ...
            if (curPoint.PowerDianLiu.Contains("Imax"))
            {
                maxWCnum = tableHeader;
            }
            //最大误差次数
          ...
          //  float _xIb = Number.GetCurrentByIb(_curPoint.PowerDianLiu, Helper.MeterDataHelper.Instance.Meter(GlobalUnit.FirstYaoJianMeter).Mb_chrIb, Helper.MeterDataHelper.Instance.Meter(GlobalUnit.FirstYaoJianMeter).Mb_BlnHgq);
            ...
            if (curPoint.PowerDianLiu.Contains("Imax"))
            {
                m_WCMaxSeconds = (int)(sleepTime * 9 * maxWCnum / 1000F + 10);
            }
            ...
            while (!m_CheckOver)
            {
                ...
                #region //分合元差
                if (CurPlan.Dif_Err_Flag == 1)
                {
                    string strHKey = "111010700";
                    if (CurPlan.PowerFangXiang == Cus_PowerFangXiang.正向有功)
                    {
                        strHKey = "111010700";
                    }
                    else if (CurPlan.PowerFangXiang == Cus_PowerFangXiang.正向无功)
                    {
                        strHKey = "131010700";
                    }
                    else if (CurPlan.PowerFangXiang == Cus_PowerFangXiang.反向有功)
                    {
                        strHKey = "121010700";
                    }
                    else if (CurPlan.PowerFangXiang == Cus_PowerFangXiang.反向无功)
                    {
                        strHKey = "141010700";
                    }
                    ...
                    for (int j = 0; j < BwCount; j++)
                    {
                        ...
                        if (_meter.MeterErrors.ContainsKey(strHKey))
                        {
                            #region 存在该数据
                            MeterError _ErrOnlyRead = _meter.MeterErrors[strHKey];
                            if (!_meter.MeterErrors.ContainsKey(ItemKey)) continue;
                            MeterError _ErrF = _meter.MeterErrors[ItemKey];
                            _ErrF.AVR_DIF_H_ERRORS = _ErrOnlyRead.Me_chrWcMore;
                            try
                            {
                                string[] onlyRead = _ErrOnlyRead.Me_chrWcMore.Split('|');
                                string[] errF = _ErrF.Me_chrWcMore.Split('|');
                                if (onlyRead.Length < 3 || errF.Length < 3) continue;
                                double value1, value2;
                                if (double.TryParse(onlyRead[2], out value1) && double.TryParse(errF[2], out value2))
                                {
                                    float[] tpmWc = new float[] { (float)(value2 - value1) };

                                    MeterError curError1 = CalculateMeterError(curPoint, meterLevel, tpmWc);
                                    string[] getValue = curError1.Me_chrWc.Split('|');
                                    _ErrF.AVR_DIF_ERR_AVG = getValue[1];
                                    _ErrF.AVR_DIF_ERR_ROUND = getValue[2];
                                    #region 获取 平衡与不平衡上下限差
                                    float limit = GetErrLimit();
                                    if (limit != 0)
                                    {
                                        _ErrF.AVR_UPPER_LIMIT = "+" + limit.ToString();
                                        _ErrF.AVR_LOWER_LIMIT = "-" + limit.ToString();
                                        if (Math.Abs(value1 - value2) > limit)
                                        {//不平衡与平衡误差限值不合格
                                            _ErrF.AVR_DIS_REASON += "非平衡超差";
                                            _ErrF.Me_chrWcJl = Variable.CTG_BuHeGe;
                                        }
                                    }
                                    else
                                    {
                                        _ErrF.AVR_UPPER_LIMIT = "-";
                                        _ErrF.AVR_LOWER_LIMIT = "-";
                                    }
                                    #endregion
                                }
                            }
                            ...
                            {
                                _ErrF.AVR_DIF_ERR_AVG = "--";
                                DataLoger.Log(ex.Message);
                            }
                            #endregion 存在该数据
...

                {
                    for (int i = 0; i < BwCount; i++)
                    {
                        ...
                        if (arrayTemp.Length > 3)
                        {
                            ...
                            ResultDictionary["误差3"][i] = arrayTemp[2];
                            ResultDictionary["误差4"][i] = arrayTemp[3];
                            ResultDictionary["误差5"][i] = arrayTemp[4];
                            ResultDictionary["误差6"][i] = arrayTemp[5];
                            ResultDictionary["误差7"][i] = arrayTemp[6];
                            ResultDictionary["误差8"][i] = arrayTemp[7];
                            ResultDictionary["误差9"][i] = arrayTemp[8];
                            ResultDictionary["误差10"][i] = arrayTemp[9];
                    ...
                    MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "误差3", ResultDictionary["误差3"]);
                    
                    MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "误差4", ResultDictionary["误差4"]);
                    
                    MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "误差5", ResultDictionary["误差5"]);
                   
                    MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "误差6", ResultDictionary["误差6"]);
                   
                    MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "误差7", ResultDictionary["误差7"]);
                   
                    MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "误差8", ResultDictionary["误差8"]);
                   
                    MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "误差9", ResultDictionary["误差9"]);
                   
                    MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "误差10", ResultDictionary["误差10"]);
            ...
            for (int i = 0; i < BwCount; i++)
            {
                ...
                float[] f5 = new float[tableHeader];//存放1个表位的5个误差值
                float maxWc = float.Parse(ResultDictionary["误差1"][i]);
                float minWc = float.Parse(ResultDictionary["误差1"][i]);



                for (int a = 2; a <= tableHeader; a++)
                {
                    if (float.Parse(ResultDictionary["误差" + a][i]) > maxWc)
                    {
                        maxWc = float.Parse(ResultDictionary["误差" + a][i]);
                    }
                    if (float.Parse(ResultDictionary["误差" + a][i]) < minWc)
                    {
                        minWc = float.Parse(ResultDictionary["误差" + a][i]);
                    }
                }
                float fc = Math.Abs(maxWc-minWc);//计算偏差估计值
                ResultDictionary["结论"][i] = fc > meterLevel ? Variable.CTG_BuHeGe : Variable.CTG_HeGe;

                ResultDictionary["偏差值"][i] = fc.ToString("0.0000");
        ...
        protected override bool CheckPara()
        {
            ...
            st_Wc.PowerFangXiang = (Cus_PowerFangXiang)Enum.Parse(typeof(Cus_PowerFangXiang), arrayErrorPara[2]);
            st_Wc.PowerYinSu = arrayErrorPara[4];
            #region 功率元件
            switch (arrayErrorPara[3])
            {
                case "H":
