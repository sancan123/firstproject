...
{
    class InfluenceVoltageRange : VerifyBase
    {
        #region ----------变量声明----------
        ...
        #region ----------构造函数----------
        public InfluenceVoltageRange(object plan)
            : base(plan)
        {
            //检测检定参数是否正确
      ...
      //      Helper.VerifyDemoHelper.Instance.Reset();
        ...
        private bool InitEquipment(StPlan_WcPoint _curPoint, float PL, int[] _Pulselap)
        {
            ...
            bool result = Helper.EquipHelper.Instance.PowerOn(GlobalUnit.U *float.Parse( _curPoint.PowerDianYa), _xIb, (int)_curPoint.PowerYuanJian, (int)_curPoint.PowerFangXiang, FangXiangStr + _curPoint.PowerYinSu, IsYouGong, false);
            if (!result)
            {
                ...
                for (int i = 1; i <= 3; i++)
                {
                    ...
                    isSuccess = Helper.EquipHelper.Instance.PowerOn(GlobalUnit.U * float.Parse(_curPoint.PowerDianYa), _xIb, (int)_curPoint.PowerYuanJian, (int)_curPoint.PowerFangXiang, FangXiangStr + _curPoint.PowerYinSu, IsYouGong, false);
                    if (isSuccess)
                    {
                        break;
                    }
        ...
        protected override bool CheckPara()
        {
            ...
            float.TryParse(arrayErrorPara[4].Replace("%", ""), out floatTemp);
            floatTemp = floatTemp / 100f;
            st_Wc.PowerDianYa = floatTemp.ToString();
