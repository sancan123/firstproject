...
namespace CLDC_VerifyAdapter
{
    ...
    /// 基本误差检定控制器
    /// </summary>
    class InitialInherentError : VerifyBase
    {
        #region ----------变量声明----------
        float xUa = 0;
        /// <summary>
        ...
        #region ----------构造函数----------
        public InitialInherentError(object plan)
            : base(plan)
        {
            //检测检定参数是否正确
            // m_VerifyDemo.BWCount = BwCount;
            ResultNames = new string[] { "测试时间", "功率元件", "功率方向", "电压倍数", "电流倍数", "功率因数", "上升误差1", "上升误差2", "上升平均值", "下降误差1", "下降误差2", "下降平均值", "差值", "结论" };
        }
        #endregion
        ...
        public override void Verify()
        {
            ...
            for (int i = 0; i < BwCount; i++)
            {
                if (Helper.MeterDataHelper.Instance.Meter(i).YaoJianYn)
                {
                    ...
                    ResultDictionary["电压倍数"][i] = arrPlanList[i].PowerDianYa;
            ...
            MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "电压倍数", ResultDictionary["电压倍数"]);
            ...
            if (arrayErrorPara[1] == "是")
            {
                StPlan_WcPoint curPoint1 = CurPlan;
                float _xIb = Number.GetCurrentByIb(curPoint1.PowerDianLiu, Helper.MeterDataHelper.Instance.Meter(GlobalUnit.FirstYaoJianMeter).Mb_chrIb, Helper.MeterDataHelper.Instance.Meter(GlobalUnit.FirstYaoJianMeter).Mb_BlnHgq);
                bool result = Helper.EquipHelper.Instance.PowerOn(GlobalUnit.U * float.Parse(curPoint1.PowerDianYa), _xIb, (int)curPoint1.PowerYuanJian, (int)curPoint1.PowerFangXiang, FangXiangStr + curPoint1.PowerYinSu, IsYouGong, false);
                if (Stop) return;
                ShowWaitMessage("正在等待{0}秒,请稍候....", 1000 * 600);
            }


            if (arrayErrorPara[0] == "否")
            {
                CLDC_DataCore.DataBase.DataControl data = new CLDC_DataCore.DataBase.DataControl(false);
                string VALUEsql = "";
                string ProjectNamesql = "";
                string[] strVALUE = new string[BwCount];
                string[] strProjectName = new string[BwCount];
                bool CheckOver = false;
                string WC1 = "";
                string WC2 = "";
                string PJZ = "";

                string WCstr = GetWCProjectName();
                string Bw = "";
                for (int i = 0; i < BwCount; i++)
                {
                    ...
                    VALUEsql = string.Format("select AVR_VALUE from TMP_METER_COMMUNICATION where FK_LNG_METER_ID  = '{0}' and AVR_PROJECT_NAME = '{1}'", Helper.MeterDataHelper.Instance.Meter(i).Mb_ID, WCstr);
                    //初始固有误差试验_是_正向有功_H_1.0_100%_10Itr
                    strVALUE[i] = data.GetData(VALUEsql);
                    WC1 = GetWCValue(strVALUE[i], 0);
                    WC2 = GetWCValue(strVALUE[i], 1);
                    PJZ = GetWCValue(strVALUE[i], 2);

                    if (string.IsNullOrEmpty(WC1) || string.IsNullOrEmpty(WC2) || string.IsNullOrEmpty(PJZ))
                    {
                        CheckOver = true;
                        Bw = (i + 1).ToString();
                    ...
                    {
                        UpPjz[i] = float.Parse(PJZ);
                        ResultDictionary["上升误差1"][i] = WC1;
                        ResultDictionary["上升误差2"][i] = WC2;
                        ResultDictionary["上升平均值"][i] = PJZ;

                    }

                }
                MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "上升误差1", ResultDictionary["上升误差1"]);
                Thread.Sleep(1000);              
                MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "上升误差2", ResultDictionary["上升误差2"]);
                Thread.Sleep(1000);
                MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "上升平均值", ResultDictionary["上升平均值"]);
                if (CheckOver)
                {
                    Helper.LogHelper.Instance.WriteInfo("表位没有测试该电流点的上升误差：" + Bw);
                    //System.Windows.Forms.MessageBox.Show("请确定以下表位有测试该电流点的上升误差:" + Bw);
                    //Stop = true;
                    //return;
                }
            ...
            int maxWCnum = tableHeader + 2;                         //最大误差次数
            ...
            while (!m_CheckOver)
            {
...

                {
                    ...
                    for (int i = 0; i < BwCount; i++)
                    {
                        ...
                        {
                            if (arrayErrorPara[0] == "是")
                            {
                                ResultDictionary["上升误差1"][i] = arrayTemp[0];
                                ResultDictionary["上升误差2"][i] = arrayTemp[1];
                                ResultDictionary["上升平均值"][i] = arrayTemp[2];

                            }
                            else
                            {
                                ResultDictionary["下降误差1"][i] = arrayTemp[0];
                                ResultDictionary["下降误差2"][i] = arrayTemp[1];
                                ResultDictionary["下降平均值"][i] = arrayTemp[2];


                                //  strProjectName[i] = data.GetData(ProjectNamesql);
                    ...
                    {
                        MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "下降误差1", ResultDictionary["下降误差1"]);
                        MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "下降误差2", ResultDictionary["下降误差2"]);
                    }
            ...
            {
                MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "下降平均值", ResultDictionary["下降平均值"]);
                ...
                for (int i = 0; i < BwCount; i++)
                {
                    ...
                    if (string.IsNullOrEmpty(ResultDictionary["上升平均值"][i]) || string.IsNullOrEmpty(ResultDictionary["上升误差1"][i]) || string.IsNullOrEmpty(ResultDictionary["上升误差2"][i]))
                    {
                        ResultDictionary["结论"][i] = "不合格";
                        ResultDictionary["差值"][i] = "";
                    }

                    if (string.IsNullOrEmpty(ResultDictionary["下降平均值"][i]) || string.IsNullOrEmpty(ResultDictionary["下降误差1"][i]) || string.IsNullOrEmpty(ResultDictionary["下降误差2"][i]))
                    {
                        ResultDictionary["结论"][i] = "不合格";
                        ResultDictionary["差值"][i] = "";
                    }
                    ...
                    {
                        ResultDictionary["差值"][i] = ((float.Parse(ResultDictionary["下降平均值"][i]) + UpPjz[i])/2).ToString("0.0000");
                        if (float.Parse(ResultDictionary["差值"][i]) > meterLevel)
                        {
                            ResultDictionary["结论"][i] = "不合格";
                        }
                    }
        ...
        private bool InitEquipment(StPlan_WcPoint _curPoint, float PL, int[] _Pulselap)
        {
            ...
            bool result = Helper.EquipHelper.Instance.PowerOn(GlobalUnit.U * xUa, _xIb, (int)_curPoint.PowerYuanJian, (int)_curPoint.PowerFangXiang, _curPoint.PowerYinSu, IsYouGong, false);
            if (!result)
            {
        ...
        protected override bool CheckPara()
        {
            ...
            st_Wc.PowerFangXiang = (Cus_PowerFangXiang)Enum.Parse(typeof(Cus_PowerFangXiang), arrayErrorPara[3]);
            ...
            st_Wc.Pc = 0;
            st_Wc.PointId = 1;
            st_Wc.PowerDianLiu = arrayErrorPara[7];
            xUa = float.Parse(arrayErrorPara[6].ToString().Trim('%')) / 100;
            st_Wc.PowerDianYa = xUa.ToString();
            st_Wc.PowerYinSu = arrayErrorPara[5];
            #region 功率元件
            switch (arrayErrorPara[4])
            {
                case "H":
            ...
            #endregion
            //  if (arrayErrorPara[9] == "是")
            //  {
            //      GlobalUnit.g_CUS.DnbData.CheckState = Cus_CheckStaute.调表;
            //  }
        ...
        */
            ...
            m_WCJump = GlobalUnit.GetConfig(Variable.CTC_WC_JUMP, 0.2F);
            //标准脉冲分频系数
        ...
        public string GetWCProjectName()
        {
            ...
            //初始固有误差试验_是_正向有功_H_1.0_100%_10Itr
            string[] arrayErrorPara = VerifyPara.Split('|');
            string WcProjectName = "初始固有误差试验_是_" + arrayErrorPara[3] + "_" + arrayErrorPara[4] + "_" + arrayErrorPara[5] + "_" + arrayErrorPara[6] + "_" + arrayErrorPara[7];
            if(arrayErrorPara[2] == "奇次谐波")
            {
                WcProjectName = "90度相位触发波形试验影响前固有误差_是_" + arrayErrorPara[3] + "_" + arrayErrorPara[4] + "_" + arrayErrorPara[5] + "_" + arrayErrorPara[6] + "_" + arrayErrorPara[7]; 
            }
            else if(arrayErrorPara[2] == "一相或两相中断")
            {
                WcProjectName = "一相或两相电压中断试验影响前固有误差_是_" + arrayErrorPara[3] + "_" + arrayErrorPara[4] + "_" + arrayErrorPara[5] + "_" + arrayErrorPara[6] + "_" + arrayErrorPara[7];
            }
             else if (arrayErrorPara[2] != "初始化误差")
            {
                WcProjectName = arrayErrorPara[2] + "试验影响前固有误差_是_" + arrayErrorPara[3] + "_" + arrayErrorPara[4] + "_" + arrayErrorPara[5] + "_" + arrayErrorPara[6] + "_" + arrayErrorPara[7];
            }
        ...
        //H^正向有功^^10Itr^1.0^-0.2849^-0.2869^-0.2859^^^^

        public string GetWCValue(string data, int state)
        {
            string[] str = null;
            string result = "";
            //H^正向有功^^10Itr^1.0^-0.2849^-0.2869^-0.2859^^^^
            if (!string.IsNullOrEmpty(data))
            {
                ...
                else
                {
                    if (state == 0)
                    {
                        result = str[5].ToString();
                    }
                    else if (state == 1)
                    {
                        result = str[6].ToString();
                    }
                    else if (state == 2)
                    {
                        result = str[7].ToString();
                    }
                }
            }
