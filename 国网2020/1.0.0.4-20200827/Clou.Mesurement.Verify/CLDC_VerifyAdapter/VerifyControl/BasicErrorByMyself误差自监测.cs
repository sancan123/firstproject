...
namespace CLDC_VerifyAdapter
{
    ...
    /// 误差自监测.
    /// </summary>
    class BasicErrorByMyself : VerifyBase
    {
        //1：误差自监测数据更新间隔时间1分钟
        //    2：检测前误差超限次数和时间
        //        3：做电表误差（功率因数1，电流IB，电压220），等待190s，
        //4：读取台体误差和电表自监测误差，
        //    5：检测后误差超限次数和时间
        //        6：判断误差次数，台体误差值和电表自监测误差
        ...
        #region ----------构造函数----------
        public BasicErrorByMyself(object plan)
            : base(plan)
        {
            //检测检定参数是否正确
            // m_VerifyDemo.BWCount = BwCount;
            ResultNames = new string[] { "测试时间", "功率方向", "电流倍数", "检测前误差超限次数", "检测前上1次事件记录发生时刻", "误差超限阀值", "电表自监误差", "检测后误差超限次数", "检测后上1次事件记录发生时刻", "不合格原因", "结论" };
        }
        #endregion
        ...
        public override void Verify()
        {
            ...
            bool[] result4444 = MeterProtocolAdapter.Instance.SouthIdentity(iFlag, out strRand1, out strRand2, out strEsamNo);
            if (Stop) return;
          
            int[] arrTimeMin = new int[BwCount];
            //计量芯误差自监测数据更新间隔时间（默认值03）
            Common.Memset(ref strData, "04000415" + "03");
            Common.Memset(ref strCode, "04000415");
            bool[] result = MeterProtocolAdapter.Instance.SouthParameterElseUpdate(iFlag, strRand2, strData, strCode);
            //误差自监测误差超限阀值
            Common.Memset(ref strData, "04091901" + "0040");
            Common.Memset(ref strCode, "04091901");
            result = MeterProtocolAdapter.Instance.SouthParameterElseUpdate(iFlag, strRand2, strData, strCode);

            //计量误差判定次数范围
            Common.Memset(ref strData, "04091902" + "01");
            Common.Memset(ref strCode, "04091902");
            result = MeterProtocolAdapter.Instance.SouthParameterElseUpdate(iFlag, strRand2, strData, strCode);
            string[] str04092202 = MeterProtocolAdapter.Instance.ReadData("04092202", 1);
            //误差自监测数据记录更新间隔时间
            Common.Memset(ref strData, "04000A10" + "0003");
            Common.Memset(ref strCode, "04000A10");
            result = MeterProtocolAdapter.Instance.SouthParameterElseUpdate(iFlag, strRand2, strData, strCode);
            //调用基类的检定方法，执行默认操作
            ...
            /*当前是否已经停止校验*/
            if (Stop) return;
             string[] strLoseCountQ = MeterProtocolAdapter.Instance.ReadData("03600000",3);
            string[] strLoseTimeQ = MeterProtocolAdapter.Instance.ReadData("03600001", 6);

            MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "检测前误差超限次数", strLoseCountQ);
            MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "检测前上1次事件记录发生时刻", strLoseTimeQ);
      ...
      //      Helper.VerifyDemoHelper.Instance.Reset();
            ...
            if (Stop) return;
            bool[] k = new bool[BwCount];
            for (int i = 0; i < BwCount; i++)
            {
                k[i] = false;
            }
            Helper.EquipHelper.Instance.SetLoadRelayControl(k, 1);
            int waitTimeForErr = 420;
         ...
         //   string[] strError1 = MeterProtocolAdapter.Instance.ReadData("02821002", 3);
       //    string[] strError2 = MeterProtocolAdapter.Instance.ReadData("02821003", 3);
         //   string[] strError3 = MeterProtocolAdapter.Instance.ReadData("02821004", 3);
       //    string[] strError4= MeterProtocolAdapter.Instance.ReadData("02821005", 3);
          
            MessageController.Instance.AddMessage("正在读取误差自监测误差数据...");
            // float[] strErrors = MeterProtocolAdapter.Instance.ReadData(strDI, 3, 1);
         

           
    
            if (Stop) return;
            string[] strLoseCountH = MeterProtocolAdapter.Instance.ReadData("03600000", 3);
            string[] strLoseTimeH= MeterProtocolAdapter.Instance.ReadData("03600001", 6);
    
            if (Stop) return;
            Helper.EquipHelper.Instance.SetLoadRelayControl(GlobalUnit.blnYaoJianMeter, 0);

            MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "检测后误差超限次数", strLoseCountH);
            MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "检测后上1次事件记录发生时刻", strLoseTimeH);
            MessageController.Instance.AddMessage("正在处理结果");
            for (int i = 0; i < BwCount; i++)
            {
                if (!Helper.MeterDataHelper.Instance.Meter(i).YaoJianYn) continue;
                float wcFZ = 0.4f;
                ResultDictionary["误差超限阀值"][i] = wcFZ.ToString();//误差1
                if ( strError[i]== "" )
                {
            ...
            MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "误差超限阀值", ResultDictionary["误差超限阀值"]);
