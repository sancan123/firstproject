...
{ 
    ...
    class ErrorVarietyErr : VerifyBase
    {
        ...
        public ErrorVarietyErr(object plan)
            : base(plan) 
        {
            ResultNames = new string[] { "测试时间", "误差(5min前1.0)", "误差(5min前0.5L)", "误差(5min后1.0)", "误差(5min后0.5L)", "(1.0)变差值", "(0.5L)变差值", "结论", "不合格原因" };
        }
       ...
       int[]     PulseClone = (int[])arrPulseLap.Clone();
            ...
            while (!m_CheckOver)
            {
...

                {
                    for (int i = 0; i < BwCount; i++)
                    {
                        ...
                        //"5min前1.0误差","5min前0.5L误差","5min后1.0误差","5min后0.5L误差","1.0变差值","0.5L变差值"
                        if (gg == 0)
                        {
                            ResultDictionary["误差(5min前1.0)"][i] = GlobalUnit.g_CUS.DnbData.MeterGroup[i].MeterErrors[ItemKey].Me_chrWcMore;
                        ...
                        {
                            ResultDictionary["误差(5min前0.5L)"][i] = GlobalUnit.g_CUS.DnbData.MeterGroup[i].MeterErrors[ItemKey].Me_chrWcMore;
                            if (arrayTemp.Length > 3)
                            {
                        ...
                        {
                            ResultDictionary["误差(5min后1.0)"][i] = GlobalUnit.g_CUS.DnbData.MeterGroup[i].MeterErrors[ItemKey].Me_chrWcMore;
                            if (arrayTemp.Length > 3)
                            {
                        ...
                        {
                            ResultDictionary["误差(5min后0.5L)"][i] = GlobalUnit.g_CUS.DnbData.MeterGroup[i].MeterErrors[ItemKey].Me_chrWcMore;
                            if (arrayTemp.Length > 3)
                            {
        ...
        //    MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "结论", ResultDictionary["结论"]);
            ...
            if (gg == 0)
            {
                MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "误差(5min前1.0)", ResultDictionary["误差(5min前1.0)"]);
            }
            if (gg == 1)
            {
                MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "误差(5min前0.5L)", ResultDictionary["误差(5min前0.5L)"]);
            }
            if (gg == 2)
            {
                MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "误差(5min后1.0)", ResultDictionary["误差(5min后1.0)"]);
            }
            if (gg == 3)
            {
                MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "误差(5min后0.5L)", ResultDictionary["误差(5min后0.5L)"]);
            }
            ...
            for (int i = 0; i < BwCount; i++)
            {
                ...
                if (FirWc1[i] == "" || FirWc5[i] == "" || SecWc1[i] == "" || SecWc5[i] == "")
                {
                    ...
                    ResultDictionary["不合格原因"][i] = "读不到误差";
                }
                ...
                else
                {
                    ...
                    ResultDictionary["(1.0)变差值"][i] = BC1[i].ToString("0.0000");
                    ResultDictionary["(0.5L)变差值"][i] = BC5[i].ToString("0.0000");

                    if (BC1[i] > 0.2 || BC5[i] > 0.2)
                    {
                        ResultDictionary["结论"][i] = "不合格";
                        ResultDictionary["不合格原因"][i] = "变差超差";
                    }
                    else
                    {
            ...
            MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "(1.0)变差值", ResultDictionary["(1.0)变差值"]);
            ShowWaitMessage("正在等待{0}秒,请稍候....", 1000 * 1);
            MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "(0.5L)变差值", ResultDictionary["(0.5L)变差值"]);
            ShowWaitMessage("正在等待{0}秒,请稍候....", 1000 * 1);
            MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "结论", ResultDictionary["结论"]);
            ShowWaitMessage("正在等待{0}秒,请稍候....", 1000 * 1);
