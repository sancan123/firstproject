...
namespace CLDC_VerifyAdapter
{
    ...
    /// 负载电流升降变差试验
    /// </summary>
    class LoadCurrentUpAndDown : VerifyBase
    {
        #region ----------变量声明----------
        ...
        #region ----------构造函数----------
        public LoadCurrentUpAndDown(object plan)
            : base(plan)
        {
            //检测检定参数是否正确
            // m_VerifyDemo.BWCount = BwCount;
            ResultNames = new string[] {"测试时间", "Imin上升误差1","Imin上升误差2","Imin上升平均值","Imin上升化整值",
                                         "Imin下降误差1","Imin下降误差2","Imin下降平均值","Imin下降化整值","Imin差值",                                       
                                         "10Itr上升误差1","10Itr上升误差2","10Itr上升平均值","10Itr上升化整值",
                                         "10Itr下降误差1","10Itr下降误差2","10Itr下降平均值","10Itr下降化整值","10Itr差值",
                                         "Imax上升误差1","Imax上升误差2","Imax上升平均值","Imax上升化整值",
                                         "Imax下降误差1","Imax下降误差2","Imax下降平均值","Imax下降化整值","Imax差值",
                                         "结论","不合格原因" };
        }
        #endregion
        ...
        public override void Verify()
        {
            ...
            for (int i = 1; i <= 6; i++)
            {
                // if (GlobalUnit.IsDan) //单相不做Imin(0.1Itr)
                //  {
                //      if (i == 1 || i == 8)
                //          continue;
                //   }
                ...
                if (i == 4)
                {
                    ...
                    bool result = Helper.EquipHelper.Instance.PowerOn(GlobalUnit.U, _xIb, (int)curPoint1.PowerYuanJian, (int)curPoint1.PowerFangXiang, FangXiangStr + curPoint1.PowerYinSu, IsYouGong, false);
                    if (Stop) return;
                    ShowWaitMessage("正在等待{0}秒,请稍候....", 1000 * WaitTime * 120);
                }
                if (Stop) return;
                Verify2(i);
            }
            ...
            for (int i = 0; i < BwCount; i++)
            {
                ...
                {
                    string Hz1 = ResultDictionary["Imin上升平均值"][i];
                    string Hz2 = ResultDictionary["Imin下降平均值"][i];

                    string Hz5 = ResultDictionary["10Itr上升平均值"][i];
                    string Hz6 = ResultDictionary["10Itr下降平均值"][i];
                    string Hz7 = ResultDictionary["Imax上升平均值"][i];
                    string Hz8 = ResultDictionary["Imax下降平均值"][i]; 


                    float VarietyErr1 = 100F;
                  
                    float VarietyErr3 = 100F;
                    float VarietyErr4 = 100F;
                    if (string.IsNullOrEmpty(Hz1) || string.IsNullOrEmpty(Hz2)  || string.IsNullOrEmpty(Hz5) || string.IsNullOrEmpty(Hz6) || string.IsNullOrEmpty(Hz7) || string.IsNullOrEmpty(Hz8))
                    {
                        ResultDictionary["结论"][i] = Variable.CTG_BuHeGe;
                        ResultDictionary["不合格原因"][i] = "误差点值为空";
                        continue;
                    }
                    if (Hz1 == "" || Hz2 == ""  || Hz5 == "" || Hz6 == "" || Hz7 == "" || Hz8 == "")
                    {
                        ResultDictionary["结论"][i] = Variable.CTG_BuHeGe;
                        ResultDictionary["不合格原因"][i] = "误差点值为空";
                        continue;
                    ...
                    {
                        VarietyErr1 = Math.Abs(float.Parse(Hz2) - float.Parse(Hz1));
                        ResultDictionary["Imin差值"][i] = VarietyErr1.ToString("0.0000");
                    }
                   
                    if (!string.IsNullOrEmpty(Hz5) && !string.IsNullOrEmpty(Hz6))
                    {
                        VarietyErr3 = Math.Abs(float.Parse(Hz6) - float.Parse(Hz5));
                        ResultDictionary["10Itr差值"][i] = VarietyErr3.ToString("0.0000");
                    }
                    if (!string.IsNullOrEmpty(Hz7) && !string.IsNullOrEmpty(Hz8))
                    {
                        VarietyErr4 = Math.Abs(float.Parse(Hz8) - float.Parse(Hz7));
                        ResultDictionary["Imax差值"][i] = VarietyErr4.ToString("0.0000");
                    }

                  
                        if (VarietyErr1 <= 0.25  && VarietyErr3 <= 0.25 && VarietyErr4 <= 0.25)
                        {
            ...
            //通知界面
            UploadTestResult("Imin差值");
            ShowWaitMessage("正在等待{0}秒,请稍候....", 1000 * 1);
          
            UploadTestResult("10Itr差值");
            ShowWaitMessage("正在等待{0}秒,请稍候....", 1000 * 1);
            UploadTestResult("Imax差值");
            ShowWaitMessage("正在等待{0}秒,请稍候....", 1000 * 1);
        ...
        public void Verify2(int count)
        {
            ...
            m_WCMaxSeconds = (int)(sleepTime * 15 * maxWCnum / 1000F + 10);
            if (sleepTime > 10000)
            {
                sleepTime = 5000;
            }       //避免等待时间过长，5秒钟读取一次误差
            ...
            while (!m_CheckOver)
            {
                ...
                for (int i = 0; i < BwCount; i++)
                {
                    ...
                    string[] ArrayCurrent = new string[] { "Imin", "10Itr", "Imax", "Imax", "10Itr", "Imin" };
                    ...
                    if (arrayTemp.Length > 3)
                    {
                        if (count <= 3)
                        {
                            ResultDictionary[ArrayCurrent[count - 1] + "上升误差1"][i] = arrayTemp[0];
                            ResultDictionary[ArrayCurrent[count - 1] + "上升误差2"][i] = arrayTemp[1];
                            ResultDictionary[ArrayCurrent[count - 1] + "上升平均值"][i] = arrayTemp[2];
                            ResultDictionary[ArrayCurrent[count - 1] + "上升化整值"][i] = arrayTemp[3];

                        }
                        else
                        {
                            ResultDictionary[ArrayCurrent[count - 1] + "下降误差1"][i] = arrayTemp[0];
                            ResultDictionary[ArrayCurrent[count - 1] + "下降误差2"][i] = arrayTemp[1];
                            ResultDictionary[ArrayCurrent[count - 1] + "下降平均值"][i] = arrayTemp[2];
                            ResultDictionary[ArrayCurrent[count - 1] + "下降化整值"][i] = arrayTemp[3];
                ...
                string[] ArrayCurrent1 = new string[] { "Imin", "10Itr", "Imax", "Imax", "10Itr", "Imin" };
                if (count <= 3)
                {

                    UploadTestResult(ArrayCurrent1[count - 1] + "上升误差1");
                    ShowWaitMessage("请等待处理", 1000 * 1);
                    UploadTestResult(ArrayCurrent1[count - 1] + "上升误差2");
                    ShowWaitMessage("请等待处理", 1000 * 1);
                    UploadTestResult(ArrayCurrent1[count - 1] + "上升平均值");
                    ShowWaitMessage("请等待处理", 1000 * 1);
                    UploadTestResult(ArrayCurrent1[count - 1] + "上升化整值");
                }
                else
                {
                    UploadTestResult(ArrayCurrent1[count - 1] + "下降误差1");
                    ShowWaitMessage("请等待处理", 1000 * 1);
                    UploadTestResult(ArrayCurrent1[count - 1] + "下降误差2");
                    ShowWaitMessage("请等待处理", 1000 * 1);
                    UploadTestResult(ArrayCurrent1[count - 1] + "下降平均值");
                    ShowWaitMessage("请等待处理", 1000 * 1);
                    UploadTestResult(ArrayCurrent1[count - 1] + "下降化整值");
                }
        ...
        /// 加载/检测检定参数是否正确
        /// </summary>
        protected bool CheckPara1(int index)
        {
            //误差试验类型|功率方向|功率元件|功率因数|电流倍数|添加谐波|逆相序|误差试验圈数|上限|下限
            //基本误差|正向有功|H|1.0|Imax|否|否

            string[] ArryCurrent = new string[] { "Imin",  "10Itr", "Imax", "Imax", "10Itr", "Imin" };
            ...
            st_Wc.Pc = 0;
            st_Wc.PointId = 1;
            st_Wc.PowerDianLiu = ArryCurrent[index - 1];
