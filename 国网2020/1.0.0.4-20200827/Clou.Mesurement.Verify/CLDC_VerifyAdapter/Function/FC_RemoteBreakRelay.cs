...
namespace CLDC_VerifyAdapter.Function
{
    ...
    /// 通断电-拉闸功能
    /// </summary>
    public class FC_RemoteBreakRelay : VerifyBase
    {
        ...
        public FC_RemoteBreakRelay(object plan)
            : base(plan)
        {
        }
        ...
        public override void Verify()
        {
            ...
            if (GlobalUnit.IsNZLoadRelayControl && GlobalUnit.IsDan)
            {
                ...
                //Common.Memset(ref strWDZETime, DateTime.Now.ToString("yyMMddHHmmss"));
                if (Stop) return;
                Helper.EquipHelper.Instance.PowerOff();
                ...
                for (int i = 0; i < BwCount; i++)
                {
                    ...
                    {
                        strMeterWDZBTime[i] = strMeterWDZInfo[i].Substring(strMeterWDZInfo.Length - 12, 12);
