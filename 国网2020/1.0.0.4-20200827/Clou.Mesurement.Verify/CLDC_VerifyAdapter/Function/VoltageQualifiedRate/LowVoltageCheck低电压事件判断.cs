...
{
    class LowVoltageCheck:VerifyBase
    {
        public LowVoltageCheck(object plan)
            : base(plan) 
        {
        ...
        public override void Verify()
        {
            ...
            PowerOn();
            if (Stop) return;
            MessageController.Instance.AddMessage("正在读取低电压事件产生前低电压事件总次数");
            string[] strLoseCountQ = MeterProtocolAdapter.Instance.ReadData("03BD0700", 3);
            ...
            Common.Memset(ref strDataCode, "04091202");
            Common.Memset(ref strData, "04091202" + writedata);
            ...
            bool ret = Helper.EquipHelper.Instance.PowerOn(GlobalUnit.U*0.5F, 0, 1, 1, "1.0", true, false);
            if (!ret)
            {
            ...
            if (Stop) return;
            MessageController.Instance.AddMessage("正在读取低电压事件产生后低电压事件总次数");
            string[] strLoseCountH = MeterProtocolAdapter.Instance.ReadData("03BD0700", 3);
            MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "事件产生后事件次数", strLoseCountH);
            MessageController.Instance.AddMessage("正在读取低电压事件产生后低电压事件发生时间");
            string[] strLoseTimeQ = MeterProtocolAdapter.Instance.ReadData("03BD0701", 6);
            ...
            iFlag = MeterProtocolAdapter.Instance.SouthCheckIdentity(out strRand1, out strRand2, out strEsamNo);
             writedata = FormatWriteData("10", "NN", 1, 0);
            Common.Memset(ref strDataCode, "04091202");
            Common.Memset(ref strData, "04091202" + writedata);
