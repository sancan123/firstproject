...
{
    class MonthVoltageQualifiedRateItem:VerifyBase
    {
        public MonthVoltageQualifiedRateItem(object plan)
            : base(plan) 
        {
        ...
        public override void Verify()
        {
            ...
            MessageController.Instance.AddMessage("正在读取本月电压合格率记录");
            string[] strLogUp = MeterProtocolAdapter.Instance.ReadData("03100000", 27);
            UpPer = GetHGL(strLogUp,36,6,"0");   
            ...
            if (Stop) return;
            MessageController.Instance.AddMessage("正在读取本月电压合格率记录");
            string[] strLogDown = MeterProtocolAdapter.Instance.ReadData("03100000", 27);
            MonitorTime = GetHGL(strLogDown, 48, 6, "2");
            VoltagePer = GetHGL(strLogDown, 42, 6, "0");
            DownPer = GetHGL(strLogDown, 36, 6, "0");
            UpPerTime = GetHGL(strLogDown, 30, 6, "2");
            DownPerTime = GetHGL(strLogDown, 24, 6, "2");
            MaxV = GetHGL(strLogDown, 20, 4, "1");
            MaxTime = GetHGL(strLogDown, 12, 8, "2");
            MinV = GetHGL(strLogDown, 8, 4, "1");
            MinTime = GetHGL(strLogDown, 0, 8, "2");
            ...
            for (int i = 0; i < BwCount; i++)
            {
                ...
                if (!string.IsNullOrEmpty(MonitorTime[i]) && !string.IsNullOrEmpty(VoltagePer[i]) && !string.IsNullOrEmpty(UpPer[i])
                    ...
                    && !string.IsNullOrEmpty(MaxV[i]) && !string.IsNullOrEmpty(MaxTime[i]) && !string.IsNullOrEmpty(MinV[i]) && !string.IsNullOrEmpty(MinTime[i]))
                {
