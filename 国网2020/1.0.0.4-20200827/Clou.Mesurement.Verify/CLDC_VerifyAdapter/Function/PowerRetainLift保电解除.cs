...
{
    class PowerRetainLift: VerifyBase
    {
        public PowerRetainLift(object plan)
            : base(plan)
        { }
        ...
        {
            ResultNames = new string[] { "测试时间", "保电状态位前一后", "结论", "不合格原因" };
            return true;
        }
        ...
        public override void Verify()
        {
            ...
            for (int i = 0; i < BwCount; i++)
            {
                ...
                if (!string.IsNullOrEmpty(status3[i]))
                {
                    ...
                    {
                        ResultDictionary["保电状态位前一后"][i] = "保电";
                    }
                    else
                    {
                        ResultDictionary["保电状态位前一后"][i] = "非保电";
                    }
                }
                else
                {
                    ResultDictionary["保电状态位前一后"][i] = "异常";
                }
            }
            MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "保电状态位前一后", ResultDictionary["保电状态位前一后"]);
            ...
            for (int i = 0; i < BwCount; i++)
            {
                ...
                if (!string.IsNullOrEmpty(status3[i]))
                {
                    ...
                    {
                        ResultDictionary["保电状态位前一后"][i] = ResultDictionary["保电状态位前一后"][i] + "-非保电";
                    }
                    else
                    {
                        ResultDictionary["保电状态位前一后"][i] = ResultDictionary["保电状态位前一后"][i] + "-保电";
                    }
                }
                else
                {
                    ResultDictionary["保电状态位前一后"][i] = ResultDictionary["保电状态位前一后"][i]+ "-异常";
                }
            }
            MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "保电状态位前一后", ResultDictionary["保电状态位前一后"]);
            if (Stop) return;
            ...
            {
                if (!Helper.MeterDataHelper.Instance.Meter(i).YaoJianYn) continue;
                if (ResultDictionary["保电状态位前一后"][i] == "保电-非保电")
                {
