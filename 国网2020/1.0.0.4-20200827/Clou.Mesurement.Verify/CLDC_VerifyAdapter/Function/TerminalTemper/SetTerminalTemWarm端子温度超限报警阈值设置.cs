...
{
    class SetTerminalTemWarm:VerifyBase
    {


           #region ----------构造函数----------

        public SetTerminalTemWarm(object plan)
            : base(plan)
        {
        }
        ...
        public override void Verify()
        {
           ...
           MessageController.Instance.AddMessage("正在读取端子温度超限报警阈值");
            string[] flt_DL = MeterProtocolAdapter.Instance.ReadData("04091201",2);
            ...
            string writedata = FormatWriteData("1200", "NNNN", 2, 0);
            Common.Memset(ref strDataCode, "04091201");
            Common.Memset(ref strData, "04091201" + writedata);
            ...
            string[] flt_Tzz2 = MeterProtocolAdapter.Instance.ReadData("04091201", 2);
            ...
            string writedata1 = FormatWriteData("1150", "NNNN", 2, 0);
            Common.Memset(ref strDataCode, "04091201");
            Common.Memset(ref strData, "04091201" + writedata1);
                 ...
                 for (int i = 0; i < BwCount; i++)
            {
                if (Helper.MeterDataHelper.Instance.Meter(i).YaoJianYn)
                {
                    ...
                    else
                    {
                        ...
                        ResultDictionary["不合格原因"][i] = "设置后阀值不符合设定值";
                    }
