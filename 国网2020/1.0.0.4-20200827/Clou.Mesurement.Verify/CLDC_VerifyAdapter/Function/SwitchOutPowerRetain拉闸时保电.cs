...
{
    class SwitchOutPowerRetain: VerifyBase
    {
        public SwitchOutPowerRetain(object plan)
            : base(plan)
        { }
        ...
        public override void Verify()
        {
            ...
            iFlag = MeterProtocolAdapter.Instance.SouthCheckIdentity(out strRand1, out strRand2, out strEsamNo);
            MessageController.Instance.AddMessage("正在下发跳闸延迟时间为0分钟,请稍候....");
            string writedata = FormatWriteData("0000", "NNNN", 2, 0);
            ...
            for (int i = 0; i < BwCount; i++)
            {
                ...
                if (!string.IsNullOrEmpty(status3[i]))
                {
                    ...
                    if (str.Substring(11, 1) == "1" && str.Substring(9, 1) == "1")
                    {
            ...
            {
                System.Windows.MessageBox.Show("请长按电表按钮3秒之后，松开，并按确定按钮");   
            }
            ...
            for (int i = 0; i < BwCount; i++)
            {
                ...
                {
                    string str = CLDC_DataCore.Function.Common.HexStrToBinStr(status3[i]);
                    if (str.Substring(11, 1) == "0" && str.Substring(9, 1) == "0" && str.Substring(3, 1) == "1")
                    {
