...
{
    class PulsePerSecond : VerifyBase
    {
        ...
        public PulsePerSecond(object plan)
            : base(plan)
        {
        ...
        {
            //初始化结论
            ResultNames = new string[] { "误差1", "误差2", "误差3", "误差4", "误差5", "误差6", "误差7", "误差8", "误差9", "误差10", "平均值", "化整值", "结论" };
        ...
        //result =     MeterProtocolAdapter.Instance.SetPulseCom(00);
        //for (int g = 0; g < BwCount; g++)
        //{
        //    if (Stop) return;                   //假如当前停止检定，则退出
        //    if (!Helper.MeterDataHelper.Instance.Meter(g).YaoJianYn)
        //    {
        //        continue;
        //    }
        //    if (result[g] == false)
        //    {
        //        ShowWaitMessage("{0}表位切换秒脉冲失败",(g+1));
        //    }
        //}
            if (Stop)
            {
            ...
            while (true)
            {
                ...
                if (ReadWCNum >= maxErrorTimes)
                {
                    for (int i = 0; i < BwCount; i++)
                    {
                        ...
                        {
                            //如果读了十次仍未读到数据，则不管
                            if (errorTable.Rows[i][0].ToString() == "")
                            {
                                continue;
                            }
        ...
        protected override void ClearItemData()
        {
            ...
            for (int i = 1; i < 4; i++)
            {
                for (int k = 0; k < BwCount; k++)
                {
                    ...
                    strCurKey = string.Format("{0}0{1}", strKey, i);
                    if (curMeter.MeterDgns.ContainsKey(strCurKey))
                    {
                        curMeter.MeterDgns.Remove(strCurKey);
                    }
                    //添加新节点
                }
            }
        ...
        protected override void DefaultItemData()
        {
            ...
            {
                curMeter = Helper.MeterDataHelper.Instance.Meter(k);
                string strKey = _Key + "01";
                MeterDgn avtResult;
                if (curMeter.MeterDgns.ContainsKey(strKey))
                    avtResult = curMeter.MeterDgns[strKey];
                else
                {
                    avtResult = new MeterDgn();
                    avtResult.Md_PrjID = strKey;
                    avtResult.Md_PrjName = Cus_DgnItem.日计时误差.ToString() + "-平均值";
                    curMeter.MeterDgns.Add(strKey, avtResult);
                }
                avtResult.Md_chrValue = "";
                //前五次误差
                strKey = _Key + "02";
                MeterDgn preResult;
                if (curMeter.MeterDgns.ContainsKey(strKey))
                    preResult = curMeter.MeterDgns[strKey];
                else
                {
                    preResult = new MeterDgn();
                    preResult.Md_PrjID = strKey;
                    preResult.Md_PrjName = Cus_DgnItem.日计时误差.ToString() + "-前五次误差";
                    curMeter.MeterDgns.Add(strKey, preResult);
                }
                preResult.Md_chrValue = "||||";
                //后五次误差
                MeterDgn afterResult;
                strKey = _Key + "03";
                if (curMeter.MeterDgns.ContainsKey(strKey))
                    afterResult = curMeter.MeterDgns[strKey];
                else
                {
                    afterResult = new MeterDgn();
                    afterResult.Md_PrjID = strKey;
                    afterResult.Md_PrjName = Cus_DgnItem.日计时误差.ToString() + "-后五次误差";
                    curMeter.MeterDgns.Add(strKey, afterResult);
                }
                afterResult.Md_chrValue = "||||";
            }
