...
namespace CLDC_VerifyAdapter.Influence
{
    ...
    /// 传导差模电流干扰试验
    /// </summary>
    class ConductionDifferentialModeCurrentInterference : VerifyBase
    {
        #region ----------变量声明----------
        ...
        #endregion
        public ConductionDifferentialModeCurrentInterference(object plan)
            : base(plan)
        {
        ...
        public override void Verify()
        {
            ...
            for (int i = 0; i < BwCount; i++)
            {
                ...
                sql = string.Format("select AVR_VALUE from TMP_METER_COMMUNICATION where FK_LNG_METER_ID  = '{0}' and AVR_PROJECT_NAME = '{1}'", Helper.MeterDataHelper.Instance.Meter(i).Mb_ID, WCstr);

                str[i] = data.GetData(sql);
            }
            string Bw = "";
            ...
            for (int i = 0; i < BwCount; i++)
            {
                ...
                {
                     BeforeWc[i] = str[i].Split('^')[2];

                     ResultDictionary["影响前误差"][i] = BeforeWc[i];
