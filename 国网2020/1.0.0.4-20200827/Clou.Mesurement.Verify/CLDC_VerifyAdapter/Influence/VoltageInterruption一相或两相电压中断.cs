...
{
    class VoltageInterruption  : VerifyBase
    {
        #region ----------变量声明----------
        ...
        #endregion
        public VoltageInterruption(object plan)
            : base(plan)
        {
            ResultNames = new string[] { "功率元件", "功率方向", "电流倍数", "功率因数", "影响前误差平均值", "影响后误差", "变差", "结论" };
        }
        ...
        public override void Verify()
        {
            ...
            for (int i = 0; i < BwCount; i++)
            {
                ...
                VALUEsql = string.Format("select AVR_VALUE from TMP_METER_COMMUNICATION where FK_LNG_METER_ID  = '{0}' and AVR_PROJECT_NAME = '{1}' and AVR_PROJECT_NAME like '%{2}%'", Helper.MeterDataHelper.Instance.Meter(i).Mb_ID, WCstr, array[4]);
             //   ProjectNamesql = string.Format("select AVR_PROJECT_NAME from TMP_METER_COMMUNICATION where FK_LNG_METER_ID  = '{0}' and AVR_PROJECT_NAME like '{1}%' and AVR_PROJECT_NAME like '%{2}%'", Helper.MeterDataHelper.Instance.Meter(i).Mb_ID, WCstr, array[4]);
                strVALUE[i] = data.GetData(VALUEsql);
              //  strProjectName[i] = data.GetData(ProjectNamesql);
            }
            string Bw = "";
            //H^正向有功^1^Imax^1.0^-0.2883^-0.2883^-0.2906^-0.2925^-0.2954^-0.2939^0.0515
            //5次谐波试验影响前固有误差_正向有功_H_1.0_10Itr-Imax-Imax-10Itr
            for (int i = 0; i < BwCount; i++)
            {
                if (Stop) return;                   //假如当前停止检定，则退出
                if (!Helper.MeterDataHelper.Instance.Meter(i).YaoJianYn)
                ...
                string strData = (strVALUE[i].Split('^')[11]);
                ResultDictionary["影响前误差平均值"][i] = strData;
                BeforeWc[i] = strData;
            }
            MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "影响前误差平均值", ResultDictionary["影响前误差平均值"]);
            if (CheckOver)
            {
              //  System.Windows.Forms.MessageBox.Show("请确定以下表位有测试影响前固有误差:" + Bw);
                Stop = true;
                return;
            }
            if (Stop) return;
            ...
            while (!m_CheckOver)
            {
...

                {
                    for (int i = 0; i < BwCount; i++)
                    {
                        ...
                        {
                            BC = (float.Parse(AfterWc[i]) - float.Parse(BeforeWc[i])).ToString("0.0000");
                        }
        ...
        */
        ...
        private bool InitEquipment(StPlan_WcPoint _curPoint, float PL, int[] _Pulselap, bool before)
        {
            ...
            if (!before) //yxian
            {
                ...
                {
                    case "A相":
                        result = Helper.EquipHelper.Instance.PowerOn(0, GlobalUnit.U, GlobalUnit.U, _xIb, _xIb, _xIb, (int)_curPoint.PowerYuanJian, 50, _curPoint.PowerYinSu, IsYouGong, false, false, (int)_curPoint.PowerFangXiang);
                        break;
                    case "B相":
                        result = Helper.EquipHelper.Instance.PowerOn(GlobalUnit.U, 0, GlobalUnit.U, _xIb, _xIb, _xIb, (int)_curPoint.PowerYuanJian, 50, _curPoint.PowerYinSu, IsYouGong, false, false, (int)_curPoint.PowerFangXiang);
                        break;
                    case "C相":
                        result = Helper.EquipHelper.Instance.PowerOn(GlobalUnit.U, GlobalUnit.U, 0, _xIb, _xIb, _xIb, (int)_curPoint.PowerYuanJian, 50, _curPoint.PowerYinSu, IsYouGong, false, false, (int)_curPoint.PowerFangXiang);
                        break;
                    case "AB相":
                        result = Helper.EquipHelper.Instance.PowerOn(0, 0, GlobalUnit.U, _xIb, _xIb, _xIb, (int)_curPoint.PowerYuanJian, 50, _curPoint.PowerYinSu, IsYouGong, false, false, (int)_curPoint.PowerFangXiang);
                        break;
                    case "AC相":
                        result = Helper.EquipHelper.Instance.PowerOn(0, GlobalUnit.U, 0, _xIb, _xIb, _xIb, (int)_curPoint.PowerYuanJian, 50, _curPoint.PowerYinSu, IsYouGong, false, false, (int)_curPoint.PowerFangXiang);
                        break;
                    case "BC相":
                        result = Helper.EquipHelper.Instance.PowerOn(GlobalUnit.U, 0, 0, _xIb, _xIb, _xIb, (int)_curPoint.PowerYuanJian, 50, _curPoint.PowerYinSu, IsYouGong, false, false, (int)_curPoint.PowerFangXiang);
                        break;
                    default:
                        result = false;
                        break;
        ...
        public string GetWCProjectName()
        {
          ...
          //  影响前误差_正向有功_H_1.0_10Itr
            ...
            string[] arrayErrorPara = VerifyPara.Split('|');
            string WcProjectName = "一相或两相电压中断试验影响前固有误差_否_" + arrayErrorPara[1] + "_" + arrayErrorPara[2] + "_" + arrayErrorPara[3] + "_" + "100%" + "_" + arrayErrorPara[4];
