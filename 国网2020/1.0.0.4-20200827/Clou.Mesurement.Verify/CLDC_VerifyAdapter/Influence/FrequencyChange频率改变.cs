...
{
    class FrequencyChange : VerifyBase
    {
        #region ----------变量声明----------
        ...
        #endregion
        public FrequencyChange(object plan)
            : base(plan)
        {
       ...
       bool  resultInitEquip = InitEquipment(curPoint, float.Parse(meterInfo.Mb_chrHz), arrPulseLap, InfluenceBefore);
        ...
        int   sleepTime = PulseClone[0] * GetOneErrorTime();   //计算一个脉冲大概需要的时间 毫秒
            ...
            //    MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "结论", ResultDictionary["结论"]);
            errorTable = null;
            EquipHelper.Instance.SettingWaveformSelection(0, 0, 0, 0, 0, 0);
        ...
        protected override bool CheckPara()
        {
            ...
            st_Wc.PL =float.Parse( arrayErrorPara[9]);
        ...
        */
        ...
        private bool InitEquipment(StPlan_WcPoint _curPoint, float PL, int[] _Pulselap, bool before)
        {
            ...
            if (!before) //yxian
            {
                result = Helper.EquipHelper.Instance.PowerOn(GlobalUnit.U, GlobalUnit.U, GlobalUnit.U, _xIb, _xIb, _xIb, (int)_curPoint.PowerYuanJian, (float)_curPoint.PL, _curPoint.PowerYinSu, IsYouGong, false, false, (int)_curPoint.PowerFangXiang);
                if (!result)
                {
        ...
        public string GetWCProjectName()
        {
          ...
          //  影响前误差_正向有功_H_1.0_10Itr
            ...
            string[] arrayErrorPara = VerifyPara.Split('|');
            string WcProjectName = "频率改变试验影响前固有误差_否_" + arrayErrorPara[1] + "_" + arrayErrorPara[2] + "_" + arrayErrorPara[3] + "_" + "100%" + "_" + arrayErrorPara[4];
