...
namespace CLDC_VerifyAdapter.Influence
{
    class HighHarmonic: VerifyBase
    {
        ...
        float xIc =  0;
        int Cs =  0;
     //   string[] dj = new string[0];

        private bool InfluenceBefore = true;

        #endregion
        public HighHarmonic(object plan)
            : base(plan) 
        {
            ResultNames = new string[] { "测试时间","功率元件", "功率方向", "电流倍数", "功率因数", "影响前误差平均值", "影响后误差", "变差", "结论","不合格原因" };
        }
        ...
        public override void Verify()
        {
          ...
          string  WCstr = GetWCProjectName();
            ...
            float[,] bcwc = new float[BwCount, 52];
            float[,] wc = new float[BwCount, 52]; 
            for (int gg = 0; gg < 52; gg++)
            {
                if (gg >= 26)
                {
                    MessageController.Instance.AddMessage("开始检测" + (66-gg) + "次谐波");
                }
                else
                {
                    MessageController.Instance.AddMessage("开始检测" + (gg + 15) + "次谐波");
                }
       ...
       int[]     PulseClone = (int[])arrPulseLap.Clone();
         ...
         int    sleepTime = PulseClone[0] * GetOneErrorTime();   //计算一个脉冲大概需要的时间 毫秒
            ...
            while (!m_CheckOver)
            {
...

                {
                    for (int i = 0; i < BwCount; i++)
                    {
                        ...
                        {
                            AfterWc[i] = arrayTemp[0];
                            wc[i, gg] =float.Parse( arrayTemp[0]);

                        }else
                        {
                              wc[i, gg] = -999;
                        }
                        if (AfterWc[i] != "" && BeforeWc[i] != "" && AfterWc[i] != null && BeforeWc[i] != null)
                        {
                            BWc[i] = (float.Parse(AfterWc[i]) - float.Parse(BeforeWc[i])).ToString("0.0000");
                            bcwc[i, gg] = float.Parse(AfterWc[i]) - float.Parse(BeforeWc[i]);
                        }else
                        {
                             bcwc[i, gg] = -999;
                        }                                    
            ...
            for (int i = 0; i < BwCount; i++)
            {
                ...
                if (Helper.MeterDataHelper.Instance.Meter(i).YaoJianYn)
                {
                    ...
                    if (gg == 0)
                    {
                        ResultDictionary["影响后误差"][i] = AfterWc[i];
                        ResultDictionary["变差"][i] = BWc[i];
                    }
                    else
                    {
                        ResultDictionary["影响后误差"][i] = ResultDictionary["影响后误差"][i] + "," + AfterWc[i];
                        ResultDictionary["变差"][i] = ResultDictionary["变差"][i] + "," + BWc[i];
                    }
                }
        ...
        //    MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "结论", ResultDictionary["结论"]);
            ...
            for (int i = 0; i < BwCount; i++)
            {
               ...
               string[] dj = Number.getDj(Helper.MeterDataHelper.Instance.Meter(i).Mb_chrBdj);
                ...
                if (dj.Length > 2)
                {
                    if (WCX == "0")
                    {
                        wcx = float.Parse(dj[0]);

                    }
                    else
                    {
                        wcx = float.Parse(dj[1]);
                    }
                }
                else
                {
                ...
                for (int d = 0; d < 52; d++)
                {
                    if (bcwc[i, d] == -999 || wc[i, d] == -999)
                    {
                        result[i] = false;
                        Re[i] = "读不到误差";
                        break;
                    }
                    else if (Math.Abs(bcwc[i, d]) > wcx || Math.Abs(wc[i, d]) > wcx)
                    {
                        result[i] = false;
                        Re[i] = "误差超差";
                        break;
            ...
            MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "结论", ResultDictionary["结论"]);
            MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "不合格原因", Re);
        ...
        protected override bool CheckPara()
        {
            ...
            if (arrayErrorPara[1].Contains('有'))
            {
                WCX = "0";
            }
            else
        ...
        */
        ...
        private bool InitEquipment(StPlan_WcPoint _curPoint, float PL, int[] _Pulselap ,bool before,int count)
        {
            ...
            if (!before) //yxian
            {
                ...
                if (GlobalUnit.clfs == Cus_Clfs.单相)
                {
                    ...
                    arrPara[0].Content[Cs] = 0.02f;
                    arrPara[0].TimeSwitch[0] = true;
                    arrPara[0].TimeSwitch[Cs] = true;
                    arrPara[0].IsOpen = true;
                    arrPara[3].Content[0] = 1F;
                    arrPara[3].Content[Cs] = Content;
                    arrPara[3].TimeSwitch[0] = true;
                    arrPara[3].TimeSwitch[Cs] = true;
                    arrPara[3].IsOpen = true;
                ...
                else
                {
                    for (int g = 0; g < 3; g++)
                    {
                        ...
                        arrPara[g].Content[Cs] = 0.02f;
                        arrPara[g].TimeSwitch[0] = true;
                        arrPara[g].TimeSwitch[Cs] = true;
                        arrPara[g].IsOpen = true;
                    }
                    for (int h = 3; h < 6; h++)
                    {
                        arrPara[h].Content[0] = 1F;
                        arrPara[h].Content[Cs] = Content;
                        arrPara[h].TimeSwitch[0] = true;
                        arrPara[h].TimeSwitch[Cs] = true;
        ...
        public string GetWCProjectName()
        {
            ...
            string[] arrayErrorPara = VerifyPara.Split('|');
            string WcProjectName = "高次谐波试验影响前固有误差_否_" + arrayErrorPara[1] + "_" + arrayErrorPara[2] + "_" + arrayErrorPara[3] + "_" + "100%" + "_" + arrayErrorPara[4];
