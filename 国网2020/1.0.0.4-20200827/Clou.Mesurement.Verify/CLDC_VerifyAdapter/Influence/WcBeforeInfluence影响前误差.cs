...
{
    class WcBeforeInfluence : VerifyBase
    {
        #region ----------变量声明----------
        ...
        string[] UpWC = new string[1];
        string[] DownWc = new string[1];
        float[] PJZ = new float[1];
        #endregion

        #region ----------构造函数----------
        public WcBeforeInfluence(object plan)
            : base(plan)
        {
            //检测检定参数是否正确
            // m_VerifyDemo.BWCount = BwCount;
            ResultNames = new string[] { "第一次误差值", "第二次误差值", "平均值", "结论" };
        }
        #endregion
        ...
        public override void Verify()
        {
            ...
            string[] CurrentArry = verPlan[5].Split('-');
            UpWC = new string[BwCount];
            DownWc = new string[BwCount];
            PJZ = new float[BwCount];
            ...
            for (int i = 0; i < BwCount; i++)
            {
                ...
                {
                    if (!string.IsNullOrEmpty(UpWC[i]) && !string.IsNullOrEmpty(DownWc[i]))
                    {

                        PJZ[i] = (Math.Abs(float.Parse(UpWC[i]) + float.Parse(DownWc[i]))) / 2;
                        ResultDictionary["平均值"][i] = PJZ[i].ToString();
            ...
            while (!m_CheckOver)
            {
                ...
                for (int i = 0; i < BwCount; i++)
                {
                    ...
                    if (arrayTemp.Length > 3)
                    {
                        if (count == 0)
                        {
                            ResultDictionary["第一次误差值"][i] = arrayTemp[0] + "|" + arrayTemp[1] + "|" + arrayTemp[2] + "|" + arrayTemp[3];
                            UpWC[i] = arrayTemp[2];
                        }
                        else
                        {
                            ResultDictionary["第二次误差值"][i] = arrayTemp[0] + "|" + arrayTemp[1] + "|" + arrayTemp[2] + "|" + arrayTemp[3];
                            DownWc[i] = arrayTemp[2];
                        }
                ...
                if (count == 0)
                {
                    MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "第一次误差值", ResultDictionary["第一次误差值"]);
                }
                else
                {
                    MessageController.Instance.UploadCheckResult(VerifyProcess.Instance.CurrentKey, "第二次误差值", ResultDictionary["第二次误差值"]);
                }
