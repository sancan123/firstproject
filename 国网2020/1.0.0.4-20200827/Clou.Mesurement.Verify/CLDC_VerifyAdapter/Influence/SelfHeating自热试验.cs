    ...
    class SelfHeating : VerifyBase
    {
        ...
        public override void Verify()
        {
            ...
            for (int i = 0; i < BwCount; i++)
            {
                ...
                VALUEsql = string.Format("select AVR_VALUE from TMP_METER_COMMUNICATION where FK_LNG_METER_ID  = '{0}' and AVR_PROJECT_NAME like '{1}%' and AVR_PROJECT_NAME like '%{2}%'", Helper.MeterDataHelper.Instance.Meter(i).Mb_ID, WCstr, array[4]);
            ...
            string Bw = "";
            //|+0.0244|+0.0231^|+0.0218|+0.0174^|+0.0231|+0.0202^|+0.0|+0.0^|+0.0188|+0.0283^|+0.0189|+0.0310^|+0.0189|+0.0296^|+0.0|+0.0^|0.0264|0.0249
            ...
            for (int i = 0; i < BwCount; i++)
            {
                ...
                string[] strPro = (strProjectName[i].Split('_'))[4].Split('-');
                for (int g = 0; g < strPro.Length; g++)
                {
                    if (strPro[g] == array[4])
                    {
                        count = g;
                ...
                string[] strData = (strVALUE[i].Split('^')[8]).Split('|');
                ResultDictionary["影响前误差平均值"][i] = strData[count + 1];
                BeforeWc[i] = strData[count + 1];
            }
       ...
       int[]     PulseClone = (int[])arrPulseLap.Clone();
         ...
         int   sleepTime = PulseClone[0] * GetOneErrorTime();   //计算一个脉冲大概需要的时间 毫秒
            ...
            while (!m_CheckOver)
            {
                ...
                string strDes = "自热运行等待时间" + (MaxTime / 60.0f).ToString("F2") + "分，已经经过" + pastMinute.ToString("F2") + "分";
        ...
        protected override bool CheckPara()
        {
            ...
            CurPlan = st_Wc;
            MaxTime = int.Parse(arrayErrorPara[9]);
            //每一个误差点取几次误差参与计算
        ...
        */
        ...
        public string GetWCProjectName()
        {
            ...
            string[] arrayErrorPara = VerifyPara.Split('|');
            string WcProjectName = "自热试验影响前固有误差_" + arrayErrorPara[1] + "_" + arrayErrorPara[2] + "_" + arrayErrorPara[3] + "_" + "100%" + "_" + arrayErrorPara[4];
