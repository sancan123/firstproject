...
namespace CLDC_DataCore.Function
{
    ...
    public static class QiDQianDFunction
    {
        ...
        /// <param name="_Clfs">测量方式</param>
        /// <returns></returns>
        public static float getQiDongTime(string GuiChengName, float U, float qIb, int MeterConst, CLDC_Comm.Enum.Cus_Clfs _Clfs,CLDC_Comm.Enum.Cus_Ywg _Ywg)
        {
            float _m = 0;
            switch (_Clfs)
            {
            ...
            else
            {
                switch (GuiChengName.ToUpper())
                {
                    ...
                    case "JJG596-2012":
                        {
                            return 80F * 1000F*60 / ((float)MeterConst * _m * U * qIb);
                        }
                    default:
                        {
                            return 1.2F * 60F * 1000F / ((float)MeterConst * _m * U * qIb);
        ...
        public static float getQianDongTime(string GuiChengName, float U, float xU, float qIb, string MeterLevel, int MeterConst, CLDC_Comm.Enum.Cus_Clfs _Clfs, CLDC_Comm.Enum.Cus_Ywg _Ywg)
        {
            ...
            if (_Ywg == CLDC_Comm.Enum.Cus_Ywg.P)
            {
                ...
                switch (GuiChengName.ToUpper())
                {
                    case "JJG307-1988":
                        {
                            return 10F * (getQiDongTime(GuiChengName, U * xU, qIb, MeterConst, _Clfs, _Ywg));
                        }
                    case "JJG307-2006":
                        {
                    ...
                    case "JJG596-2012":
                        {
                            return 100 * 1000*60 / (1.1F * fbase * (float)MeterConst * _m * U * CLDC_DataCore.Const.GlobalUnit.Imin);
                        }
                    default:
                        {
                            return 10F * (getQiDongTime(GuiChengName, U * xU, qIb, MeterConst, _Clfs, _Ywg));
            ...
            else
            {
                switch (GuiChengName.ToUpper())
                {
                    ...
                    case "JJG596-2012":
                        {
                            return 600*1000000 / ((float)MeterConst * 1 * U * CLDC_DataCore.Const.GlobalUnit.Imax);
                        }
                    default:
                        {
                            return 10F * (getQiDongTime(GuiChengName, U * xU, qIb, MeterConst, _Clfs, _Ywg));
