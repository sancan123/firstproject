...
namespace CLDC_DataCore.Function
{
    ...
    public class Number
    {
        ...
        public static float GetAvgA(float[] arrNumber, float inviade)
        {
            ...
            float Sum = 0;
            if (0 == intCount) return Const.Variable.WUCHA_INVIADE;
        ...
        /// <param name="arrNumber">输入数据数组</param>
        /// <returns>返回一组数据的偏差值((未化整))</returns>
        public static float GetWindage(string[] arrNumber)
        {
            if(arrNumber.Length<=0)
                return 0;
            float []arr=new float[arrNumber.Length];
            for (int i = 0; i < arrNumber.Length;i++ )
            {
                arr[i] = float.Parse(arrNumber[i]);
            }
            return GetWindage(arr, -999F);
        }
        /// <summary>
        /// 计算一组数据的标准偏差
        /// </summary>
        ...
        public static string[] getDj(string DjString)
        {
            ...
            if (_Dj.Length == 1)
                return new string[] { float.Parse(_Dj[0]).ToString("0.0"), float.Parse(_Dj[0]).ToString("0.0") };
            else
                return new string[] { float.Parse(_Dj[0]).ToString("0.0"), float.Parse(_Dj[1]).ToString("0.0") };
        }
        /// <summary>
        /// 电量换算时间返回时间为(分钟)
        /// </summary>
