...
namespace CLDC_DataCore.Model.CarrierProtocol
{
    ...
    public class CarrierProtocolInfo
    {
        ...
        /// 是否是宽带
        /// </summary>
        private bool m_str_isCheck_Model;

        private bool m_is_IsCheckSM;


        private string m_WxParam;


        private string m_ZbParame;
        ...
        public bool IsCheck_Model
        {
            get { return m_str_isCheck_Model; }
            set { m_str_isCheck_Model = value; }
        }
        /// <summary>
        /// 是否双模
        /// </summary>
        public bool Is_IsCheckSM
        {
            get { return m_is_IsCheckSM; }
            set { m_is_IsCheckSM = value; }
        }

        /// <summary>
        /// 载波标示
        /// </summary>
        public string ZbParame
        {
            get { return m_ZbParame; }
            set { m_ZbParame = value; }
        }
        /// <summary>
        /// 无线标示
        /// </summary>
        ...
        public static CarrierProtocolInfo GetCarrierProtocolInfo(string p_str_ProtocolName)
        {
            ...
            foreach (XmlNode node_item in doc.DocumentElement.ChildNodes)
            {
                if (node_item.Attributes["CarrierName"].Value == p_str_ProtocolName)
                {
                    ...
                    if (node_item.Attributes.Count > 9)
                    {
                        carrierInfo.IsCheck_Model = bool.Parse(node_item.Attributes["IsCheck_Model"].Value);
                    }
                    return carrierInfo;
                }
            }
        ...
        private void LoadXmlData(string p_str_ProtocolName)
        {
            ...
            doc1.Load(string.Format(System.Windows.Forms.Application.StartupPath + "\\Xml\\CarrierProtocol.xml"));
            XmlElement nodeProtocol = (XmlElement)(doc1.SelectSingleNode("CarrierProtocol/Protocols"));
            CLDC_DataCore.DataBase.clsXmlControl xmlNode = new CLDC_DataCore.DataBase.clsXmlControl(nodeProtocol);
            ...
            if (p_str_ProtocolName != "")
            {
                xnd_FindXmlNode = CLDC_DataCore.DataBase.clsXmlControl.FindSencetion(xmlNode.toXmlNode()
                                                                , CLDC_DataCore.DataBase.clsXmlControl.XPath(string.Format("R,Name,{0}", p_str_ProtocolName)));
            }
            ...
            this.m_str_ProtocolName = p_str_ProtocolName;                                                           //协议名称 

            this.m_str_CarrierType = CLDC_DataCore.DataBase.clsXmlControl.getNodeValue(xnd_FindXmlNode, "CarrierType");                                 //协议类型

            this.m_str_ReadType = CLDC_DataCore.DataBase.clsXmlControl.getNodeValue(xnd_FindXmlNode, "RdType");                                  //抄表器类型

            this.m_str_CommuType = CLDC_DataCore.DataBase.clsXmlControl.getNodeValue(xnd_FindXmlNode, "CommuType");                                       //通讯方式

            this.m_str_BaudRate = CLDC_DataCore.DataBase.clsXmlControl.getNodeValue(xnd_FindXmlNode, "BaudRate");                                //波特率

            this.m_str_ComPort = CLDC_DataCore.DataBase.clsXmlControl.getNodeValue(xnd_FindXmlNode, "Comm");                                     //端口号

            this.m_int_CmdTime = int.Parse(CLDC_DataCore.DataBase.clsXmlControl.getNodeValue(xnd_FindXmlNode, "CmdTime"));                       //命令延时(ms)

            this.m_int_ByteTime = int.Parse(CLDC_DataCore.DataBase.clsXmlControl.getNodeValue(xnd_FindXmlNode, "ByteTime"));                     //字节延时(ms)


            this.RouterID = byte.Parse(CLDC_DataCore.DataBase.clsXmlControl.getNodeValue(xnd_FindXmlNode, "RouterID"));


            var temp = CLDC_DataCore.DataBase.clsXmlControl.getNodeValue(xnd_FindXmlNode, "IsCheck_SM");
            this.Is_IsCheckSM = bool.Parse(CLDC_DataCore.DataBase.clsXmlControl.getNodeValue(xnd_FindXmlNode, "IsCheck_SM"));


            this.WxParam = CLDC_DataCore.DataBase.clsXmlControl.getNodeValue(xnd_FindXmlNode, "WxParam");


            this.ZbParame = CLDC_DataCore.DataBase.clsXmlControl.getNodeValue(xnd_FindXmlNode, "ZBParame");
            //     string ss = "";
            //    CLDC_DataCore.DataBase.clsXmlControl.LoadXml(path + CLDC_DataCore.Const.Variable.CONST_CARRIER,out ss);
            ...
            if (xnd_FindXmlNode == null) return;
            #endregion----------------------------------------------------------------------------------------------------------------
        }
