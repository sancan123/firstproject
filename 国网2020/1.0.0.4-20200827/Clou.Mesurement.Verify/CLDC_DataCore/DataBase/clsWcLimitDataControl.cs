...
namespace CLDC_DataCore.DataBase
{
    ...
    public class clsWcLimitDataControl
    {
        ...
        private void OpenDb()
        {
            if (!System.IO.File.Exists(CLDC_DataCore.Function.File.GetPhyPath(Const.Variable.CONST_WCLIMIT)))
            {
                CLDC_DataCore.DataBase.CreateMdb NewMdb = new CLDC_DataCore.DataBase.CreateMdb(CLDC_DataCore.Function.File.GetPhyPath(Const.Variable.CONST_WCLIMIT));
            ...
            _Conn.ConnectionString = CONST_ACCESS + CLDC_DataCore.Function.File.GetPhyPath(Const.Variable.CONST_WCLIMIT);
            try
            {
                _Conn.Open();
            }
                                                   ...
                                                   , string[] Arr_Column)
        {
            ...
            for (int i = 0; i < Arr_Row.Length; i++)      //循环电流倍数
            {
                ...
                for (int j = 0; j < Arr_Column.Length; j++)
                {
                    ...
                    int _IbID = int.Parse(Const.GlobalUnit.g_SystemConfig.xIbDic.getxIbID(Arr_Row[i]));

                    int _GlysID = int.Parse(Const.GlobalUnit.g_SystemConfig.GlysZiDian.getGlysID(Arr_Column[j]));
        ...
        private static string getdz002(string xIb, CLDC_Comm.Enum.Cus_PowerYuanJian Yj, string glys, bool Hgq)
        {
            ...
            if ((int)Yj == 1)
            {
                ...
                {
                    _WcLimit = "0.02";
                    if (xIb.ToLower().IndexOf("itr") > -1 || xIb.ToLower().IndexOf("imin") >= 0 )
                    {
                        _WcLimit = "0.04";
                    }
            ...
            else
            {
                ...
                else if (glys != "1.0" && (xIb.ToLower().IndexOf("imax") == -1 && xIb.ToLower().IndexOf("imax-ib") == -1 && float.Parse(xIb.ToLower().Replace("ib", "")) < 0.5F))
                    _WcLimit = "0.04";
            }
            return _WcLimit;
        ...
        private static string getdz02(string xIb, CLDC_Comm.Enum.Cus_PowerYuanJian Yj, string glys, bool Hgq)
        {
            ...
            if (glys == "1.0")
                if (Hgq)
                {
                    ...
                    else if (xIb.ToLower().IndexOf("imax") == -1 && xIb.ToLower().IndexOf("imax-ib") == -1 && float.Parse(xIb.ToLower().Replace("ib", "")) < 0.05F)
                        _WcLimit = "0.4";
                    else
                        _WcLimit = "0.2";
                }
        ...
        {
            string _WcLimit = "";
            if (xIb.ToLower() == "ib" || xIb.ToLower() == "itr")
            {
                xIb = "1.0ib";
            }
            if (glys == "1.0")
