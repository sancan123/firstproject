...
namespace CLDC_DataCore.SystemModel.Item
{
    ...
    /// </summary>
    /// <summary>
    /// Edit：增加双模检定 time：20180621 man：lee
    /// </summary>
    public class csCarrier
    {
        ...
        public void Load()
        {
            ...
            for (int _i = 0; _i < _XmlNode.ChildNodes.Count; _i++)
            {
                ...
                if (_XmlNode.ChildNodes[_i].Attributes.Count > 9)
                {
                    //sci_CarrierInfo.isCheck_Model = false;
                    //if (_XmlNode.ChildNodes[_i].Attributes[9].Value == "1")
                    //{
                    sci_CarrierInfo.IsCheck_Model = bool.Parse(_XmlNode.ChildNodes[_i].Attributes[9].Value);
                    //}

                }
                if (_XmlNode.ChildNodes[_i].Attributes.Count > 10)
                {
                    sci_CarrierInfo.IsCheck_SM = bool.Parse(_XmlNode.ChildNodes[_i].Attributes[10].Value);
                }
                if (_XmlNode.ChildNodes[_i].Attributes.Count > 11)
                {

                    sci_CarrierInfo.ZBParame = _XmlNode.ChildNodes[_i].Attributes[11].Value;
                }
                if (_XmlNode.ChildNodes[_i].Attributes.Count > 12)
                {
                    sci_CarrierInfo.WXParame = _XmlNode.ChildNodes[_i].Attributes[12].Value;
                }
        ...
        public void Save()
        {
            ...
            foreach (StCarrierInfo _ci in m_Dic_CarrierInfo.Values)
            {
                ...
                xmlControl.appendchild("", "R",
                                    ...
                                    "IsCheck_Model",
                                    _ci.IsCheck_Model.ToString(),

                                     "IsCheck_SM",
                                    _ci.IsCheck_SM.ToString(),
                                         "ZBParame",
                                    _ci.ZBParame == null ? "00" : _ci.ZBParame,
                                         "WXParame",
                                    _ci.WXParame == null ? "00" : _ci.WXParame);
            }
