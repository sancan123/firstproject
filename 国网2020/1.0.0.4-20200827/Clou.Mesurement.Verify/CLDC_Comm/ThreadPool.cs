        ...
        protected void DoLoopWork()
        {
            while (IsKeepAlive)
            {
                ...
                catch (Exception ex)
                {
                    //写日志
#if DEBUG
                    System.Windows.Forms.MessageBox.Show(string.Format("线程池发生错误!\r\n{0}\r\n具体情况请查看日志文件", ex.Message), "线程池发生错误!", System.Windows.Forms.MessageBoxButtons.OK, System.Windows.Forms.MessageBoxIcon.Error);
                    
                    DataLoger.Log(string.Format("线程池发生错误!\r\n{0}\r\n具体情况请查看日志文件", ex.Message + ex.StackTrace));
#endif
                }
            }
        }
